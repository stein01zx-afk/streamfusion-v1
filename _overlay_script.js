...
function currentViewSettingsKey(){
    ...
    const recentOverlayMessageKeys = new Map();
    const recentOverlayMessageWindowMs = 1200;
    function normalizeOverlayMessagePiece(value){ return String(value ?? '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/\s+/g, ' ').trim(); }
    function buildOverlayMessageKey(item, kind){ const platform = normalizeOverlayMessagePiece(item?.platform || 'tiktok'); const uniqueId = normalizeOverlayMessagePiece(item?.uniqueId || item?.username || item?.user || item?.displayName || ''); const message = normalizeOverlayMessagePiece(item?.message || item?.text || item?.comment || ''); const action = normalizeOverlayMessagePiece(item?.action || item?.type || kind || 'chat'); const gift = normalizeOverlayMessagePiece(item?.gift || item?.giftImage || item?.sticker || item?.stickerImage || ''); const amount = normalizeOverlayMessagePiece(item?.amount || ''); const stamp = Math.floor(Number(item?.timestamp || Date.now()) / 1000); return [kind || 'chat', platform, uniqueId, action, message, gift, amount, stamp].join('|'); }
    function shouldSkipOverlayDuplicate(item, kind = 'chat'){ const now = Date.now(); for(const [key, ts] of recentOverlayMessageKeys){ if(now - ts > recentOverlayMessageWindowMs * 6) recentOverlayMessageKeys.delete(key); } const key = buildOverlayMessageKey(item, kind); if(!key) return false; const lastSeen = recentOverlayMessageKeys.get(key); if(lastSeen && now - lastSeen < recentOverlayMessageWindowMs) return true; recentOverlayMessageKeys.set(key, now); return false; }
