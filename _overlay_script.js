...
function voiceBotSummaryHtml(){
  ...
  return chips
    ...
    function voiceFilterLabel(value){
      ...
      if (filter === "supporters") return "Solo club de fans";
    ...
    function trackVoiceActivity(item, assignment = null){
      ...
      const voiceRecord = assignment ? {
        ...
        targetKey: normalizeMatchKey(assignment.targetKey || assignment.ruleLabel || assignment.targetLabel || giftName),
        targetLabel: assignment.targetLabel || assignment.ruleLabel || giftName,
      ...
      for (const key of keys) {
        ...
        if (voiceRecord) {
          bucket.voice = { ...voiceRecord };
          if (!Array.isArray(bucket.voiceHistory)) bucket.voiceHistory = [];
          bucket.voiceHistory.push({ ...voiceRecord });
          if (bucket.voiceHistory.length > 20) bucket.voiceHistory = bucket.voiceHistory.slice(-20);
        }
      }
      saveActivityBadges();
    }
...
function voiceTokenLooksGibberish(token){
  ...
  const hasCommonPattern = /(ch|ll|rr|qu|gue|gui|que|qui|cia|cio|cion|sio|sion|bra|bre|cri|tra|pro|pre|con|com|des|del|mar|per|tor|tar|mon|bol|pen|jod|mier|put|ver|cul|sexo|semen|porn|cog|coj)/i.test(letters);
...
function cleanVoiceText(text, { isName = false } = {}){
  ...
  if (isName) {
    out = out.replace(/[^\p{L}\p{N}\sÁÉÍÓÚÜÑáéíóúüñ]/gu, " ");
  } else {
...
function resolveActivityVoiceAssignment(item){
  ...
  const buildAssignment = (assignment, fallbackLabel = "", fallbackImage = "") => {
    if (!assignment || !(assignment.voiceKey in voiceCatalog)) return null;
    const ruleId = String(assignment.ruleId || "");
    if (!ruleId || !activeRuleIds.has(ruleId)) return null;
    const targetLabel = String(assignment.label || fallbackLabel || entry?.lastGift?.name || assignment.targetLabel || "Regalo").trim() || "Regalo";
    const targetKey = normalizeMatchKey(assignment.targetKey || assignment.label || entry?.lastGift?.name || item?.gift || item?.giftName || item?.giftAlt || item?.giftId || "");
    return {
      voiceKey: assignment.voiceKey,
      mode: String(assignment.mode || "unlock"),
      ruleId,
      ruleLabel: String(assignment.label || targetLabel),
      targetKey,
      targetLabel,
      targetImage: String(assignment.targetImage || fallbackImage || entry?.lastGift?.image || ""),
      platform,
      kind: String(assignment.kind || "gift"),
      triggerAt: Number(assignment.updatedAt || assignment.triggerAt || entry?.lastGift?.updatedAt || Date.now()),
      manual: false,
      source: "activity",
    };
  };
  const history = Array.isArray(entry.voiceHistory) ? [...entry.voiceHistory] : [];
  for (let i = history.length - 1; i >= 0; i--) {
    const candidate = buildAssignment(history[i]);
    if (candidate) return candidate;
  }
  const current = buildAssignment(entry.voice, entry?.lastGift?.name || "Regalo");
...
function voiceBotActiveTabButtons(){
    ...
    function shouldVoiceRead(item){
      ...
      if (voiceBot.filter === "custom") return Boolean(cleanName && cleanMessage && assignment);
...
function currentViewSettingsKey(){
    ...
    function badgeEmoji(key, platform) {  const lower = String(key || "").toLowerCase();  if (roleBadges[lower]) return roleBadges[lower].emoji;  if (lower === "mod") return roleBadges.moderator.emoji;  if (lower === "broadcaster") return roleBadges.broadcaster.emoji;  if (lower === "sub" || lower === "subscriber") return roleBadges.subscriber.emoji;  if (lower === "vip") return roleBadges.vip.emoji;  if (lower === "verified") return roleBadges.verified.emoji;  if (lower === "staff") return roleBadges.staff.emoji;  if (lower === "founder") return roleBadges.founder.emoji;  if (lower === "premium") return roleBadges.premium.emoji;  if (lower === "member" || lower.includes("fanclub") || lower.includes("superfan")) return "❤️‍🔥";  if (lower === "tiktok") return roleBadges.tiktok.emoji;  if (lower === "twitch") return roleBadges.twitch.emoji;  if (lower.includes("mod")) return roleBadges.moderator.emoji;  if (lower.includes("vip")) return roleBadges.vip.emoji;  if (lower.includes("sub")) return roleBadges.subscriber.emoji;  if (lower.includes("member") || lower.includes("fanclub") || lower.includes("superfan")) return "❤️‍🔥";  return platform === "tiktok" ? "🎵" : "🟣"; }

function badgeText(key) {  const lower = String(key || "").toLowerCase();  if (lower.includes("broadcaster")) return "Broadcaster";  if (lower.includes("mod")) return "Mod";  if (lower.includes("vip")) return "VIP";  if (lower.includes("sub")) return "Sub";  if (lower.includes("staff")) return "Staff";  if (lower.includes("verified")) return "Verified";  if (lower.includes("founder")) return "Founder";  if (lower.includes("premium")) return "Premium";  if (lower.includes("tiktok")) return "TikTok";  if (lower.includes("twitch")) return "Twitch";  if (lower.includes("member") || lower.includes("fanclub") || lower.includes("superfan")) return "Club";  return lower.replace(/[_-]+/g, " ").replace(/\b\w/g, (m) => m.toUpperCase()); }
function badgeChips(raw, platform){ const keys = normalizeBadgeKeys(raw); if(!state.settings.personal.showBadges) return ""; const style = state.settings.personal.badgeStyle || "emoji"; return keys.map((key) => `<span class="badge">${ESC(style === "compact" ? badgeText(key) : badgeEmoji(key, platform))}</span>`).join(""); }
