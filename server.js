...
io.on("connection", (socket) => {
  ...
  socket.on("connectTikTok", async (username, ack) => {
    try {
      ...
      socket.emit("system", { message: `TikTok conectado con @${clean}.` });
      if (typeof ack === "function") ack({ ok: true, platform: "tiktok", username: clean });
    } catch (err) {
  ...
  socket.on("connectTwitch", async (channel, ack) => {
    try {
      ...
      socket.emit("system", { message: `Twitch conectado a ${clean}.` });
      if (typeof ack === "function") ack({ ok: true, platform: "twitch", channel: clean });
    } catch (err) {
