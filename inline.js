...
function findMatchingVoiceRule(item){
    ...
    function renderVoiceRuleTargets(){
      ensureGiftCatalog().then(() => {
        if (!document.getElementById("overlayVoiceTargetGrid")) return;
        renderVoiceRuleTargetsInternal();
      });
    }
    function renderVoiceRuleTargetsInternal(){
      ...
      if (voiceRuleDraft.kind === "gift") {
        ...
        if (query) {
          const qNorm = normalizeMatchKey(query);
          items = items.filter((item) => giftSearchText(item).includes(qNorm));
        }
        counter.textContent = `${items.length} resultados`;
        grid.innerHTML = items.slice(0, 120).map((item) => {
          const visibleName = giftDisplayName(item);
          const originalName = String(item?.name || item?.alt || item?.key || item?.id || "Regalo");
          const image = normalizeImageSource(item?.image || item?.icon || item?.thumb || item?.url || item?.imageUrl || "");
          const active = normalizeMatchKey(voiceRuleDraft.targetKey) === normalizeMatchKey(item?.id || item?.key || item?.name || item?.alt || "");
          const targetPayload = { key: item?.id || item?.key || item?.name || item?.alt || "", label: visibleName, originalLabel: originalName, image };
          return `<button type="button" class="overlayVoiceTargetCard ${active ? 'is-active' : ''}" data-voice-target='${esc(JSON.stringify(targetPayload))}'>${image ? `<img class="overlayVoiceTargetThumb" src="${esc(image)}" alt="${esc(visibleName)}">` : `<div class="overlayVoiceTargetThumb" aria-hidden="true"></div>`}<span class="overlayVoiceTargetText"><strong>${esc(visibleName)}</strong><span>${esc(originalName !== visibleName ? originalName + " · " : "")}${esc(item?.id || item?.coins || item?.coinsName || '')}</span></span></button>`;
        }).join("");
        if (!items.length) grid.innerHTML = `<div class="overlayVoiceHelp">No se encontró ningún regalo con ese nombre en español o inglés.</div>`;
        return;
      }
    ...
    function renderVoiceRuleRail(){
      ...
      rail.innerHTML = rules.map((rule) => {
        ...
        const gift = rule.kind === "gift" ? lookupGiftCatalog(rule.targetKey || rule.targetLabel || "") : null;
        const visibleLabel = gift ? giftDisplayName(gift) : (rule.targetLabel || rule.targetKey || 'Regla');
        const detail = [rule.platform === "twitch" ? "Twitch" : "TikTok", modeLabel, voice.label].join(" · ");
        return `<article class="overlayVoiceRuleCard" data-rule-id="${esc(rule.id)}"><div class="overlayVoiceRuleCardHeader"><div class="overlayVoiceRuleCardTitle"><strong>${badge} ${esc(visibleLabel)}</strong><span>${esc(detail)}</span></div><span class="overlayVoiceRuleBadge">${esc(status)}</span></div><div class="overlayVoiceRuleCardActions"><button type="button" data-rule-toggle="${esc(rule.id)}">${rule.active ? 'Pausar' : 'Activar'}</button><button type="button" data-rule-delete="${esc(rule.id)}">Eliminar</button></div></article>`;
      }).join("");
    }
...
function currentViewSettingsKey(){
    ...
    let giftLocaleIndex = new Map();
    async function ensureGiftCatalog() {
      if (giftCatalogPromise) return giftCatalogPromise;
      giftCatalogPromise = Promise.all([
        fetch("/data/tiktok-gifts.json").then(async (res) => { if (!res.ok) throw new Error("gift catalog load failed"); return res.json(); }),
        fetch("/data/tiktok-gift-locales-es.json").then(async (res) => res.ok ? res.json() : ({ items: {} })).catch(() => ({ items: {} }))
      ])
        .then(([data, localeData]) => {
          const items = Array.isArray(data) ? data : Array.isArray(data?.items) ? data.items : [];
          const localeItems = localeData?.items && typeof localeData.items === "object" ? localeData.items : {};
          giftLocaleIndex = new Map();
          for (const [key, label] of Object.entries(localeItems)) {
            const normalizedKey = normalizeGiftKey(key);
            if (normalizedKey && String(label || "").trim()) giftLocaleIndex.set(normalizedKey, String(label).trim());
          }
          giftCatalogItems = items.map((item) => {
            const internalKey = normalizeGiftKey(item?.key || item?.id || item?.name || item?.alt || "");
            const localized = String(item?.displayNameEs || item?.nameEs || giftLocaleIndex.get(internalKey) || "").trim();
            return localized ? { ...item, displayNameEs: localized } : item;
          });
          giftCatalogIndex = new Map();
          for (const item of giftCatalogItems) {
            const keys = [item?.id, item?.key, item?.name, item?.alt, item?.displayNameEs].map(normalizeGiftKey).filter(Boolean);
            for (const key of keys) if (!giftCatalogIndex.has(key)) giftCatalogIndex.set(key, item);
          }
          return giftCatalogItems;
        })
        .catch(() => { giftCatalogIndex = new Map(); giftCatalogItems = []; giftLocaleIndex = new Map(); return []; });
      return giftCatalogPromise;
    }
    function giftDisplayName(item) {
      if (!item) return "Regalo";
      const key = normalizeGiftKey(item?.key || item?.id || item?.name || item?.alt || "");
      return String(item?.displayNameEs || item?.nameEs || giftLocaleIndex.get(key) || item?.name || item?.alt || item?.key || item?.id || "Regalo").trim() || "Regalo";
    }
    function giftSearchText(item) {
      if (!item) return "";
      return normalizeGiftKey([item?.name, item?.alt, item?.displayNameEs, item?.nameEs, item?.key, item?.id].filter(Boolean).join(" "));
    }
