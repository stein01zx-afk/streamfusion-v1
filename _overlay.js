    ...
    function normalizeVoiceBotState(bot){
      ...
      return {
        ...
        power: {
          ...
          amount: Math.max(1, Number(source.power?.amount||1)||1), pointCost: Math.max(1, Number(source.power?.pointCost||1000)||1000), activity:["like","share","follow","moderator"].includes(String(source.power?.activity||""))?String(source.power.activity):"follow",
...
function findMatchingVoiceRule(item){
    ...
    function syncVoicePowerUI(){
      ...
      document.querySelectorAll("[data-power-activity]").forEach(b=>b.classList.toggle("is-active",b.dataset.powerActivity===p.activity));
      ...
      if(summary){
        ...
        else if(p.source==="activity") trigger=`${p.activity==='follow'?'👤 Seguidor':p.activity==='moderator'?'🛡️ Moderador':p.activity==='like'?'❤️ Likes':'🗣️ Compartir'} · ${Number(p.amount||1).toLocaleString('es-PE')}`;
    ...
    function bindVoicePowerUI(){
      ...
      document.getElementById("overlayVoicePowerActivityAmount")?.addEventListener("change",e=>update("amount",Math.max(1,Number(e.target.value)||1)));
      document.getElementById("overlayVoicePowerPointCost")?.addEventListener("change",e=>update("pointCost",Math.max(1,Number(e.target.value)||1)));
      ...
      document.getElementById("overlayVoicePowerBitsCustom")?.addEventListener("change",e=>update("bitsAmount",Math.max(1,Number(e.target.value)||1)));
