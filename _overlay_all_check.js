      ...
      try {
        ...
        for(const voice of (data?.voices || [])) {
          ...
          voiceCatalog[key] = {
            ...
            id,
            aliases: [voice?.label || '', voice?.fishId || '', voice?.id || ''].filter(Boolean),
    ...
    function normalizeVoiceBotState(bot){
      ...
      return {
        ...
        power: {
          enabled: source.power?.enabled === true, source: ["gift","points","activity"].includes(String(source.power?.source||"")) ? String(source.power.source) : "gift",
          platform: String(source.power?.platform||"tiktok").toLowerCase()==="twitch" ? "twitch" : "tiktok", targetKey:String(source.power?.targetKey||""), targetLabel:String(source.power?.targetLabel||""),
          amount: Math.max(1, Number(source.power?.amount||1)||1), pointCost: Math.max(1, Number(source.power?.pointCost||1000)||1000), activity:["like","subscription","follow","moderator"].includes(String(source.power?.activity||""))?String(source.power.activity):"follow",
          commandPrefix:String(source.power?.commandPrefix||".").slice(0,4) || ".", consumePoints: source.power?.consumePoints !== false
        },
        powerUsers: Array.isArray(source.powerUsers) ? source.powerUsers.map((u)=>({platform: normalizeVoicePlatform(u?.platform||"tiktok"), username: String(u?.username||u?.uniqueId||u?.displayName||"").trim(), displayName:String(u?.displayName||u?.username||"Usuario").trim(), active:u?.active!==false, badge:"🔥", grantedAt:Number(u?.grantedAt||Date.now()), source:String(u?.source||""), points:Number(u?.points||0)})).filter(u=>u.username) : [],
...
function findMatchingVoiceRule(item){
    ...
    function syncVoicePowerUI(){
      const p=voiceBot.power||{}; const set=(id,v)=>{const e=document.getElementById(id);if(e)e.value=String(v??"");}; const chk=(id,v)=>document.getElementById(id)?.classList.toggle("is-active",!!v);
      set("overlayVoicePowerPrefix",p.commandPrefix||"."); set("overlayVoicePowerSource",p.source||"gift"); set("overlayVoicePowerPlatform",p.platform||"tiktok"); set("overlayVoicePowerActivity",p.activity||"follow"); set("overlayVoicePowerAmount",p.amount||1); set("overlayVoicePowerPointCost",p.pointCost||1000); set("overlayVoicePowerTarget",p.targetLabel||p.targetKey||"");
      chk("overlayVoicePowerOnBtn",p.enabled===true); chk("overlayVoicePowerOffBtn",p.enabled!==true);
      chk("overlayVoicePowerConsumeOnBtn",p.consumePoints!==false); chk("overlayVoicePowerConsumeOffBtn",p.consumePoints===false);
      const summary=document.getElementById("overlayVoicePowerSummary"); const list=document.getElementById("overlayVoicePowerUsersList"); if(summary||list){ const users=(voiceBot.powerUsers||[]).filter(u=>u.active!==false); const html=users.length?users.slice(0,12).map(u=>`<span>🔥 ${esc(u.displayName||u.username)} <small>· ${u.platform==='twitch'?'Twitch':'TikTok'}</small></span>`).join(""):'<span class="overlayVoiceHelp">Todavía no hay usuarios con 🔥 en este LIVE.</span>'; if(summary)summary.innerHTML=users.length?`<div class="overlayVoicePowerUsers">${html}${users.length>12?`<span>+${users.length-12} más</span>`:""}</div>`:'<span class="overlayVoiceHelp">Todavía no hay usuarios con 🔥.</span>'; if(list) list.innerHTML=users.length?`<div class="overlayVoicePowerUsers">${html}</div>`:'<span class="overlayVoiceHelp">Todavía no hay usuarios con 🔥 en este LIVE.</span>'; }
    }
    function bindVoicePowerUI(){
      ...
      const ids=["overlayVoicePowerPrefix","overlayVoicePowerSource","overlayVoicePowerPlatform","overlayVoicePowerActivity","overlayVoicePowerAmount","overlayVoicePowerPointCost"]; ids.forEach(id=>document.getElementById(id)?.addEventListener("input",()=>{ voiceBot.power=voiceBot.power||{}; const field={overlayVoicePowerPrefix:'commandPrefix',overlayVoicePowerSource:'source',overlayVoicePowerPlatform:'platform',overlayVoicePowerActivity:'activity',overlayVoicePowerAmount:'amount',overlayVoicePowerPointCost:'pointCost'}[id]; voiceBot.power[field]=document.getElementById(id).value; saveVoiceBot(); syncVoicePowerUI(); }));
      document.getElementById("overlayVoicePowerOnBtn")?.addEventListener("click",()=>{voiceBot.power=voiceBot.power||{};voiceBot.power.enabled=true;saveVoiceBot();syncVoicePowerUI();});
      document.getElementById("overlayVoicePowerOffBtn")?.addEventListener("click",()=>{voiceBot.power=voiceBot.power||{};voiceBot.power.enabled=false;saveVoiceBot();syncVoicePowerUI();});
      document.getElementById("overlayVoicePowerConsumeOnBtn")?.addEventListener("click",()=>{voiceBot.power=voiceBot.power||{};voiceBot.power.consumePoints=true;saveVoiceBot();syncVoicePowerUI();});
      document.getElementById("overlayVoicePowerConsumeOffBtn")?.addEventListener("click",()=>{voiceBot.power=voiceBot.power||{};voiceBot.power.consumePoints=false;saveVoiceBot();syncVoicePowerUI();});
      ...
      document.getElementById("overlayVoicePowerTarget")?.addEventListener("input",()=>{const e=document.getElementById("overlayVoicePowerTarget");voiceBot.power=voiceBot.power||{};voiceBot.power.targetKey=String(e?.value||"").trim();voiceBot.power.targetLabel=String(e?.value||"").trim();saveVoiceBot();});
      syncVoicePowerUI();
    }
    ...
    function hasVoicePower(item){ const key=voicePowerUserKey(item); if(!key) return false; if(item?.voicePower===true) return true; return (voiceBot.powerUsers||[]).some((u)=>`${normalizeVoicePlatform(u?.platform||"tiktok")}:${normalizeUsername(u?.username||u?.uniqueId||u?.displayName||"")}`===key && u.active!==false); }
    ...
    function shouldVoiceRead(item){
      ...
      const powerIntent = parseVoicePowerCommand(rawVoice);
...
function fetchVoiceAudio(text, voiceId, emotion = ""){
    ...
    function queueVoiceMessage(item){
      ...
      const powerIntent = parseVoicePowerCommand(rawMessage);
      if(powerIntent.used){
...
function currentViewSettingsKey(){
    ...
    function itemHtml(item, kind){
      ...
      const overlayFontMode = String(isGift ? (p.overlayGiftFont || "inherit") : (p.overlayEventFont || "inherit"));
      const overlayFontValue = usesChatPresentation ? (isGift ? fontFamily(p.overlayGiftFont || p.font || 'inter') : (isChat ? fontFamily(p.font || 'inter') : fontFamily(p.overlayEventFont || p.font || 'inter'))) : (overlayFontMode === "inherit" ? fontFamily(p.font || "inter") : fontFamily(overlayFontMode));
    ...
    function render(){
      ...
      if (view !== 'chat') {
        ...
        const feedState=overlayFeedScrollStates[key];
        ...
        const newestChanged=newestKey!==feedState.latestKey; feedState.latestKey=newestKey;
        if(filtered.length){
          const shouldFollow=!feedState.initialized||feedState.pinned||(newestChanged&&overlayShouldFollowNew(feedState));
          if(shouldFollow){
            feedState.programmatic=true;
            requestAnimationFrame(()=>{scrollToEdge(list,layout,direction,false);requestAnimationFrame(()=>{feedState.programmatic=false;feedState.initialized=true;feedState.pinned=true;feedState.manual=false;});});
          } else {
            requestAnimationFrame(()=>{
              if(layout==='horizontal') list.scrollLeft=Math.min(feedState.left||0,Math.max(0,list.scrollWidth-list.clientWidth));
              else list.scrollTop=Math.min(feedState.top||0,Math.max(0,list.scrollHeight-list.clientHeight));
    ...
    socket.on('voiceLibraryChanged', async (payload) => {
      try {
        const list = Array.isArray(payload?.voices) ? payload.voices : [];
        for (const key of Object.keys(voiceCatalog)) {
          const v = voiceCatalog[key];
          if (String(v?.library || '').toLowerCase() === 'fish' || String(key).startsWith('fish:')) delete voiceCatalog[key];
        }
        for (const voice of list) {
          const key = `fish:${String(voice?.fishId || voice?.id || '').trim()}`;
          if (key === 'fish:') continue;
          voiceCatalog[key] = {
            label: String(voice?.label || voice?.name || voice?.fishId || voice?.id || key).trim(),
            id: String(voice?.fishId || voice?.id || '').trim(),
            aliases: [voice?.label || '', voice?.name || '', voice?.fishId || '', voice?.id || ''].filter(Boolean),
            library: 'fish',
            referenceId: String(voice?.fishId || voice?.id || '').trim()
          };
        }
        rebuildVoiceCatalogSearchIndex();
        voiceBot = normalizeVoiceBotState(voiceBot);
        ...
        syncVoiceBotUI();
        render();
      } catch (e) { console.warn('voiceLibraryChanged', e); }
    });
    ...
    socket.on('accountState', (account) => {
      ...
      if (previous.live === true && !nextLive) {
        if (state.activityBadges?.[platform]) state.activityBadges[platform] = {};
