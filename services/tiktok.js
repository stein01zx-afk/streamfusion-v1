...
function emitEvent(io, event, ownerId = connectionOwnerId) {
    ...
    const payload = {
        ...
        action: clean(event.action, type === "share" ? "Compartió" : type === "follow" ? "Comenzó a seguir" : type === "like" ? "Like" : type === "join" ? "Se unió al directo" : "Evento"),
        user: safeUser,
...
export async function connect(username, io, ownerId = "") {
    ...
    connection.on(E.MEMBER, async (data) => {
        ...
        emitEventActive({
            ...
            group: "event",
            emoji: "👻",
            action: "Se unió al directo",
            user: nickname,
            uniqueId,
            avatar: await avatarFor(data, nickname, uniqueId),
            badges,
            message: `${nickname} se unió al directo`
        });
    });
