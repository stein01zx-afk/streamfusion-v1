...
function actorDisplayName(candidate) {
    if (!candidate || typeof candidate !== "object") return "";
    return firstValidIdentity([
        candidate?.displayName, candidate?.display_name, candidate?.nickname, candidate?.nickName,
        candidate?.user?.displayName, candidate?.user?.nickname,
        candidate?.userDetails?.displayName, candidate?.userDetails?.nickname
    ]);
}
...
function emitEvent(io, event, ownerId = connectionOwnerId) {
    const isShare = String(event?.type || "").toLowerCase() === "share";
    ...
    const payload = {
        ...
        action: (() => {
            ...
            if (t === "share") return "Compartió";
            if (t === "follow") return "Empezó a seguir";
            if (t === "like") return "Dio like";
            if (t === "join" || t === "member") return "Se unió al directo";
            return clean(event.action, "Evento");
        })(),
        user: safeUser,
        ...
        userProfile: event.userProfile !== undefined ? event.userProfile : undefined,
        sourceEvent: event.sourceEvent !== undefined ? event.sourceEvent : undefined,
        ...
        connectionId: event.connectionId || connectionSessionId,
        share: undefined,
        group: event.group || (["like", "join", "follow", "share"].includes(String(event.type || "").toLowerCase()) ? "event" : (String(event.type || "").toLowerCase() === "gift" ? "gift" : (String(event.type || "").toLowerCase() === "event" ? "event" : undefined))),
        eventId: event.eventId || undefined
    };
    const enrichedPayload = globalThis.__STREAMFUSION_POINTS_HOOK__?.(ownerId, payload) || payload;
...
function socialEventKey(type, data, uniqueId = "") {
    const kind = String(type || "social").toLowerCase();
    const sourceId = clean(
        data?.msgId ?? data?.messageId ?? data?.eventId ?? data?.shareId ?? data?.socialId,
        ""
    );
    if (sourceId) return `${kind}:${sourceId}`;
    const actorId = normalizeUsername(uniqueId || actorUniqueId(data) || actorNickname(data));
    const created = clean(data?.createTime ?? data?.timestamp ?? "", "");
    const marker = clean(data?.displayType ?? data?.display_type ?? data?.label ?? data?.action ?? "", "");
    return `${kind}:${actorId}:${created}:${marker}`;
}

function rememberSocialEvent(type, data, uniqueId = "") {
    const key = socialEventKey(type, data, uniqueId);
    if (!key || key.endsWith(":")) return false;
    const now = Date.now();
    for (const [seenKey, at] of recentSocialEvents) {
        if (now - at > 15000) recentSocialEvents.delete(seenKey);
    }
    if (recentSocialEvents.has(key)) return true;
    recentSocialEvents.set(key, now);
    return false;
}

function socialKindFromPayload(data = {}, forcedType = "") {
    const forced = String(forcedType || "").toLowerCase();
    ...
    if (/share|shared|compart/.test(text)) return "share";
    if (/follow|followed|seguir|siguió|siguio/.test(text)) return "follow";
    return "";
}

function socialPresentation(type) {
    switch (String(type || "").toLowerCase()) {
        case "like": return { action: "Dio like", emoji: "❤️", message: "dio like" };
        case "join": return { action: "Se unió al directo", emoji: "👻", message: "se unió al directo" };
        case "follow": return { action: "Empezó a seguir", emoji: "👤", message: "empezó a seguir" };
        case "share": return { action: "Compartió", emoji: "🗣️", message: "compartió el LIVE" };
        default: return { action: "Evento", emoji: "✨", message: "" };
    }
}

function canonicalSocialPayload(type, data, actor, avatar, badges, ownerId = connectionOwnerId) {
    const normalizedType = String(type || "").toLowerCase();
    const presentation = socialPresentation(normalizedType);
    // TikTok's nickname is the visible profile name; uniqueId is only the identifier/handle.
    const displayName = firstValidIdentity([
        actor?.nickname, actor?.displayName, actor?.name,
        actor?.user?.nickname, actor?.user?.displayName
    ]);
    const uniqueId = firstValidIdentity([
        actor?.uniqueId, actor?.username,
        actor?.user?.uniqueId, actor?.user?.username
    ]);
    const safeDisplayName = displayName || uniqueId || "Usuario";
    const safeUniqueId = uniqueId || normalizeUsername(safeDisplayName);
    const safeAvatar = clean(avatar, "");
    const identityKey = normalizeUsername(safeUniqueId || safeDisplayName).toLowerCase();
    const sourceId = clean(data?.msgId ?? data?.messageId ?? data?.eventId ?? data?.shareId ?? data?.socialId ?? "", "");
    ...
    return {
        ...
        profilePictureUrls: safeAvatar ? [safeAvatar] : [],
        userProfile: { displayName: safeDisplayName, nickname: safeDisplayName, uniqueId: safeUniqueId, username: safeUniqueId, avatarUrl: safeAvatar },
        badges: withConfiguredModeratorBadge(badges, safeUniqueId, ownerId),
        message: `${safeDisplayName} ${presentation.message}`.trim(),
        source: "event",
        connectionId: connectionSessionId,
        eventId: sourceId || `${normalizedType}:${identityKey}:${Date.now()}`,
        sourceEvent: clean(data?.displayType ?? data?.label ?? data?.action ?? "", "")
    };
}
...
async function handleShareEvent(io, data, isActive = () => true, emitEventFn = emitEvent, emitStatsFn = emitStats) {
    ...
    const shareActor = extractShareActor(data);
    const picked = pickUser(data, "share");
    const actor = {
        uniqueId: firstValidIdentity([
            shareActor?.uniqueId,
            actorUniqueId(data),
            picked?.uniqueId
        ]),
        nickname: firstValidIdentity([
            shareActor?.nickname,
            shareActor?.displayName,
            actorNickname(data),
            picked?.nickname
        ]),
        username: firstValidIdentity([
            shareActor?.uniqueId,
            actorUniqueId(data),
            picked?.uniqueId
        ]),
        user: shareActor?.user || picked?.user || data?.user || data
    };
    if (!actor.uniqueId && actor.nickname) actor.uniqueId = normalizeUsername(actor.nickname);
    if (!actor.nickname && actor.uniqueId) actor.nickname = actor.uniqueId;

    const shareId = actor.uniqueId || normalizeUsername(actor.nickname);
    if (rememberSocialEvent("share", data, shareId)) return;

    const badges = collectBadges(data, actor.user || data?.user || data?.shareUser || data?.userDetails || data);
    const avatar = clean(
        shareActor?.avatar ||
        getAvatarFromUserObject(actor.user) ||
        getAvatarFromUserObject(data) ||
        await avatarFor(data, actor.nickname || shareId, shareId),
        ""
    );

    sessionStats.shares += 1;
    const payload = canonicalSocialPayload("share", data, actor, avatar, badges, connectionOwnerId);
    if (!isActive()) return;
    emitEventFn(io, payload);
    if (isActive()) emitStatsFn(io);
}

async function handleFollowEvent(io, data, isActive = () => true, emitEventFn = emitEvent, emitStatsFn = emitStats) {
    if (!isActive()) return;

    const picked = pickUser(data, "follow");
    const actor = {
        uniqueId: firstValidIdentity([actorUniqueId(data), picked?.uniqueId]),
        nickname: firstValidIdentity([actorNickname(data), actorDisplayName(data), picked?.nickname]),
        username: firstValidIdentity([actorUniqueId(data), picked?.uniqueId]),
        user: picked?.user || data?.user || data
    };
    if (!actor.uniqueId && actor.nickname) actor.uniqueId = normalizeUsername(actor.nickname);
    if (!actor.nickname && actor.uniqueId) actor.nickname = actor.uniqueId;

    const followId = actor.uniqueId || normalizeUsername(actor.nickname);
    if (rememberSocialEvent("follow", data, followId)) return;

    const badges = collectBadges(data, actor.user || data?.user || data?.userDetails || data);
    const avatar = await avatarFor(data, actor.nickname || followId, followId);

    sessionStats.followers += 1;
    const payload = canonicalSocialPayload("follow", data, actor, avatar, badges, connectionOwnerId);
    if (!isActive()) return;
    emitEventFn(io, payload);
    if (isActive()) emitStatsFn(io);
}
...
async function handleSocialEvent(io, data, forcedType = null, isActive = () => true, emitEventFn = emitEvent, emitStatsFn = emitStats) {
    ...
    const kind = socialKindFromPayload(data, forcedType);
    if (kind === "share") return handleShareEvent(io, data, isActive, emitEventFn, emitStatsFn);
    if (kind === "follow") return handleFollowEvent(io, data, isActive, emitEventFn, emitStatsFn);

    // Unknown social payloads are intentionally ignored. They must not become
    // fake `system / Acción social` events and must never interfere with chat.
}
