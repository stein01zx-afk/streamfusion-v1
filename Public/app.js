...
const els = {
  ...
  openVoicebotBtn: $("openVoicebotBtn"),
  closeVoicebotBtn: $("closeVoicebotBtn"),
  voicebotModal: $("voicebotModal"),
  voicebotRulesList: $("voicebotRulesList"),
  voicebotPickerList: $("voicebotPickerList"),
  voicebotPickerLabel: $("voicebotPickerLabel"),
  voicebotPickerHint: $("voicebotPickerHint"),
  voicebotSelectionSummary: $("voicebotSelectionSummary"),
  voicebotSelectionThumb: $("voicebotSelectionThumb"),
  voicebotSelectionTitle: $("voicebotSelectionTitle"),
  voicebotSelectionDesc: $("voicebotSelectionDesc"),
  voicebotSearchInput: $("voicebotSearchInput"),
  voicebotVoiceSelect: $("voicebotVoiceSelect"),
  voicebotReadMode: $("voicebotReadMode"),
  voicebotSayDice: $("voicebotSayDice"),
  voicebotIgnoreSpecial: $("voicebotIgnoreSpecial"),
  voicebotIgnoreEmotes: $("voicebotIgnoreEmotes"),
  voicebotSpanishOnly: $("voicebotSpanishOnly"),
  voicebotSaveRuleBtn: $("voicebotSaveRuleBtn"),
  resetVoicebotBtn: $("resetVoicebotBtn"),
  saveVoicebotBtn: $("saveVoicebotBtn"),
  toastWrap: $("toastWrap"),
...
function migrateSettings(settingsObj) {
  ...
  if (!p.voicebot || typeof p.voicebot !== "object") p.voicebot = {};
  if (p.voicebot.mode === undefined) p.voicebot.mode = "custom";
  if (p.voicebot.sayDiceBeforeMessage === undefined) p.voicebot.sayDiceBeforeMessage = true;
  if (p.voicebot.ignoreSpecialChars === undefined) p.voicebot.ignoreSpecialChars = true;
  if (p.voicebot.ignoreEmotes === undefined) p.voicebot.ignoreEmotes = true;
  if (p.voicebot.spanishOnly === undefined) p.voicebot.spanishOnly = true;
  if (!Array.isArray(p.voicebot.rules)) p.voicebot.rules = [];
  return s;
}
...
function renderItem(item, kind) {
  ...
  const voicebotBadge = kind === "chat" ? voicebotBadgeMarkup(item) : "";
      ...
      <div class="entryBody">
        <div class="entryBubble ${bubbleFrame}">
          <div class="entryTop">
            ...
            ${(activityBadges || voicebotBadge) ? `<span class="entryActivityBadges">${activityBadges || ""}${voicebotBadge ? voicebotBadge : ""}</span>` : ""}
...
const VOICEBOT_STICKER_PATTERNS = /[\u3040-\u30ff\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff\uac00-\ud7af]/;
const VOICEBOT_SPECIAL_FILTER = /[@_><^#~*`|\\[\]{}()/\\]/g;

const VOICEBOT_TIKTOK_EVENTS = [
  { key: "follow", label: "Seguidor", emoji: "👤" },
  { key: "like", label: "Like", emoji: "❤️" },
  { key: "share", label: "Compartir", emoji: "🗣" },
  { key: "join", label: "Primera unión", emoji: "👻" },
  { key: "member", label: "Miembro", emoji: "⭐" },
  { key: "superfan", label: "Super fan", emoji: "🌟" },
  { key: "fanclub", label: "Club de fans", emoji: "💎" },
  { key: "system", label: "Sistema", emoji: "📣" },
];

const VOICEBOT_TIKTOK_ROLES = [
  { key: "broadcaster", label: "Streamer", emoji: "👑" },
  { key: "moderator", label: "Moderador", emoji: "🛡️" },
  { key: "vip", label: "VIP", emoji: "💎" },
  { key: "subscriber", label: "Suscriptor", emoji: "⭐" },
  { key: "founder", label: "Fundador", emoji: "🏁" },
  { key: "verified", label: "Verificado", emoji: "✅" },
  { key: "superfan", label: "Super fan", emoji: "🌟" },
  { key: "member", label: "Miembro", emoji: "⭐" },
];

const VOICEBOT_TWITCH_EVENTS = [
  { key: "follow", label: "Seguidor", emoji: "👤" },
  { key: "sub", label: "Suscripción", emoji: "⭐" },
  { key: "resub", label: "Re-suscripción", emoji: "⭐" },
  { key: "bits", label: "Bits", emoji: "💎" },
  { key: "raid", label: "Raid", emoji: "⚡" },
  { key: "host", label: "Host", emoji: "⚡" },
  { key: "join", label: "Unión", emoji: "👻" },
  { key: "system", label: "Sistema", emoji: "📣" },
];

const VOICEBOT_TWITCH_ROLES = [
  { key: "broadcaster", label: "Streamer", emoji: "👑" },
  { key: "moderator", label: "Moderador", emoji: "🛡️" },
  { key: "vip", label: "VIP", emoji: "💎" },
  { key: "subscriber", label: "Suscriptor", emoji: "⭐" },
...
const voicebotRuntime = {
  ...
  speaking: false,
  activeOnce: new Map(),
  activeAlways: { tiktok: new Set(), twitch: new Set() },
  donorUsers: { tiktok: new Set(), twitch: new Set() },
};

function voicebotSettings() {
  if (!state.settings.personal.voicebot || typeof state.settings.personal.voicebot !== "object") {
    state.settings.personal.voicebot = structuredClone(defaults.personal.voicebot);
  }
  const cfg = state.settings.personal.voicebot;
  if (!Array.isArray(cfg.rules)) cfg.rules = [];
  if (cfg.mode === undefined) cfg.mode = "custom";
  if (cfg.sayDiceBeforeMessage === undefined) cfg.sayDiceBeforeMessage = true;
  if (cfg.ignoreSpecialChars === undefined) cfg.ignoreSpecialChars = true;
  if (cfg.ignoreEmotes === undefined) cfg.ignoreEmotes = true;
  if (cfg.spanishOnly === undefined) cfg.spanishOnly = true;
  return cfg;
}

function normalizeVoicebotKey(value) {
  return String(value ?? "")
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
function getVoicebotLibrary(platform, triggerType) {
  const p = String(platform || "tiktok").toLowerCase();
  const t = String(triggerType || "gift").toLowerCase();
  if (p === "twitch" && t === "event") return VOICEBOT_TWITCH_EVENTS;
  if (p === "twitch" && t === "role") return VOICEBOT_TWITCH_ROLES;
  if (p === "twitch" && t === "gift") {
    return [
      { key: "bits-10", label: "10 Bits", emoji: "💎", amount: 10 },
      { key: "bits-100", label: "100 Bits", emoji: "💎", amount: 100 },
      { key: "bits-500", label: "500 Bits", emoji: "💎", amount: 500 },
      { key: "bits-1000", label: "1000 Bits", emoji: "💎", amount: 1000 },
    ];
  }
  if (p === "tiktok" && t === "event") return VOICEBOT_TIKTOK_EVENTS;
  if (p === "tiktok" && t === "role") return VOICEBOT_TIKTOK_ROLES;
  return [];
}

function triggerTypeLabel(platform, triggerType) {
  const p = String(platform || "tiktok").toLowerCase();
  const t = String(triggerType || "gift").toLowerCase();
  if (p === "twitch" && t === "gift") return "Bits";
  if (t === "gift") return "Regalos";
  if (t === "event") return "Eventos";
  if (t === "role") return "Roles";
  return "Acciones";
}

function triggerButtonLabel(platform, triggerType) {
  const p = String(platform || "tiktok").toLowerCase();
  const t = String(triggerType || "gift").toLowerCase();
  if (p === "twitch" && t === "gift") return "Leer x bits";
  if (t === "gift") return "Leer x regalo";
  if (t === "event") return "Leer x evento";
  if (t === "role") return "Leer x rol";
  return "Seleccionar";
}

function voicebotDefaultVoice() {
  return voicebotRuntime.voices[0]?.name || "";
}

function refreshVoicebotVoices() {
  const voices = window.speechSynthesis?.getVoices?.() || [];
  voicebotRuntime.voices = Array.isArray(voices) ? voices.filter(Boolean) : [];
  voicebotRuntime.loadedVoices = true;
  if (els.voicebotVoiceSelect) {
    const current = els.voicebotVoiceSelect.value;
    const options = [
      `<option value="">Voz automática</option>`,
      ...voicebotRuntime.voices.map((voice) => `<option value="${ESC(voice.name)}">${ESC(voice.name)}${voice.lang ? ` (${ESC(voice.lang)})` : ""}${voice.default ? " — predeterminada" : ""}</option>`),
    ];
    els.voicebotVoiceSelect.innerHTML = options.join("");
    if (current && [...els.voicebotVoiceSelect.options].some((opt) => opt.value === current)) {
      els.voicebotVoiceSelect.value = current;
    } else if (!els.voicebotVoiceSelect.value) {
      els.voicebotVoiceSelect.value = "";
    }
  }
}
...
function openVoicebotModal() {
  voicebotSettings();
  renderVoicebotModal();
  openModal(els.voicebotModal);
  refreshVoicebotVoices();
}

function closeVoicebotModal() {
  closeModal(els.voicebotModal);
}

function normalizeVoicebotRule(rule) {
  const platform = String(rule?.platform || "tiktok").toLowerCase() === "twitch" ? "twitch" : "tiktok";
  const triggerType = String(rule?.triggerType || "gift").toLowerCase();
  const library = getVoicebotLibrary(platform, triggerType);
  const triggerKey = String(rule?.triggerKey || "").trim();
  const matched = library.find((entry) => normalizeVoicebotKey(entry.key) === normalizeVoicebotKey(triggerKey) || normalizeVoicebotKey(entry.label) === normalizeVoicebotKey(triggerKey));
  return {
    id: String(rule?.id || `voicebot_${Date.now()}_${Math.random().toString(16).slice(2, 8)}`),
    platform,
    triggerType,
    triggerId: String(rule?.triggerId || matched?.id || "").trim(),
    triggerKey: matched?.key || triggerKey,
    triggerLabel: matched?.label || rule?.triggerLabel || triggerKey || "Sin título",
    triggerEmoji: matched?.emoji || rule?.triggerEmoji || (triggerType === "gift" ? "🎁" : "✨"),
    triggerImage: String(rule?.triggerImage || matched?.image || "").trim(),
    triggerCoins: Number(rule?.triggerCoins ?? matched?.coins ?? 0) || 0,
    readMode: rule?.readMode === "always" ? "always" : "once",
    voiceName: String(rule?.voiceName || "").trim(),
    sayDiceBeforeMessage: rule?.sayDiceBeforeMessage !== false,
    ignoreSpecialChars: rule?.ignoreSpecialChars !== false,
    ignoreEmotes: rule?.ignoreEmotes !== false,
    spanishOnly: rule?.spanishOnly !== false,
    enabled: rule?.enabled !== false,
  };
}

function loadVoicebotToUI() {
  if (els.voicebotModal?.classList.contains("show")) {
    renderVoicebotModal();
  }
}

function renderVoicebotModal() {
  const cfg = voicebotSettings();
  els.voicebotSayDice.checked = cfg.sayDiceBeforeMessage !== false;
  els.voicebotIgnoreSpecial.checked = cfg.ignoreSpecialChars !== false;
  els.voicebotIgnoreEmotes.checked = cfg.ignoreEmotes !== false;
  els.voicebotSpanishOnly.checked = cfg.spanishOnly !== false;
  els.voicebotReadMode.value = voicebotRuntime.currentDraft?.readMode || "once";
  if (!voicebotRuntime.currentDraft) {
    voicebotRuntime.currentDraft = normalizeVoicebotRule({
      platform: voicebotRuntime.currentPlatform,
      triggerType: voicebotRuntime.currentTrigger,
      readMode: "once",
      sayDiceBeforeMessage: true,
      ignoreSpecialChars: true,
      ignoreEmotes: true,
  ...
  renderVoicebotRulesList();
  renderVoicebotPicker();
  updateVoicebotSelectionSummary();
  syncVoicebotButtons();
}

function syncVoicebotButtons() {
  const cfg = voicebotSettings();
  for (const btn of document.querySelectorAll("[data-voicebot-mode]")) {
    btn.classList.toggle("active", btn.dataset.voicebotMode === (cfg.mode || "custom"));
  }
  for (const btn of document.querySelectorAll("[data-voicebot-platform]")) {
    btn.classList.toggle("active", btn.dataset.voicebotPlatform === voicebotRuntime.currentPlatform);
  }
  for (const btn of document.querySelectorAll("[data-voicebot-trigger]")) {
    btn.classList.toggle("active", btn.dataset.voicebotTrigger === voicebotRuntime.currentTrigger);
  }
  if (els.voicebotSaveRuleBtn) {
    els.voicebotSaveRuleBtn.textContent = triggerButtonLabel(voicebotRuntime.currentPlatform, voicebotRuntime.currentTrigger);
    els.voicebotSaveRuleBtn.classList.toggle("successBtn", Boolean(voicebotRuntime.currentDraft?.triggerKey));
  }
}

function renderVoicebotRulesList() {
  const cfg = voicebotSettings();
  const rules = cfg.rules.map(normalizeVoicebotRule);
  els.voicebotRulesList.innerHTML = rules.length ? rules.map((rule) => {
    const modeText = rule.readMode === "always" ? "Leer todos" : "Leer un mensaje";
    const voiceText = rule.voiceName ? `• ${ESC(rule.voiceName)}` : "• Voz automática";
    const active = rule.enabled !== false ? "is-active" : "";
    const image = normalizeImageSource(rule.triggerImage);
    const thumb = rule.triggerType === "gift" && image
      ? `<img class="voicebotRuleThumb" src="${ESC(image)}" alt="${ESC(rule.triggerLabel || "Regalo")}" loading="lazy" onerror="this.style.display='none'">`
      : `<div class="voicebotRuleThumb" aria-hidden="true">${ESC(rule.triggerEmoji || "🤖")}</div>`;
    return `
      <article class="voicebotRuleCard ${active}">
        ${thumb}
        <div class="voicebotRuleMain">
          <p class="voicebotRuleTitle">${ESC(rule.triggerLabel || rule.triggerKey || "Sin título")}</p>
          <div class="voicebotRuleMeta">
            <span>${ESC(triggerTypeLabel(rule.platform, rule.triggerType))}</span>
            <span>${ESC(modeText)}</span>
            <span>${ESC(voiceText)}</span>
            ${rule.triggerType === "gift" && rule.triggerCoins ? `<span>• ${ESC(rule.triggerCoins)} coins</span>` : ""}
            ${rule.sayDiceBeforeMessage !== false ? `<span>• dice</span>` : ""}
            ${rule.ignoreSpecialChars !== false ? `<span>• sin símbolos</span>` : ""}
            ${rule.ignoreEmotes !== false ? `<span>• sin emotes</span>` : ""}
            ${rule.spanishOnly !== false ? `<span>• solo español</span>` : ""}
          </div>
        </div>
        <div class="voicebotRuleActions">
          <button class="iconBtn" type="button" data-voicebot-delete="${ESC(rule.id)}" ${rules.length <= 0 ? "disabled" : ""}>🗑</button>
        </div>
      </article>`;
  }).join("") : `
    <div class="emptyState">
      <strong>Aún no hay acciones</strong>
      <span>Agrega TikTok o Twitch para crear reglas de lectura.</span>
    </div>`;
}


function updateVoicebotSelectionSummary(draft = null) {
  if (!els.voicebotSelectionSummary) return;
  const selected = draft || voicebotRuntime.currentDraft || normalizeVoicebotRule({
    platform: voicebotRuntime.currentPlatform,
    triggerType: voicebotRuntime.currentTrigger,
  });
  const platform = selected.platform === "twitch" ? "Twitch" : "TikTok";
  const typeLabel = triggerTypeLabel(selected.platform, selected.triggerType);
  const isGift = selected.triggerType === "gift";
  const title = selected.triggerLabel && selected.triggerKey ? selected.triggerLabel : `Ningún ${typeLabel.toLowerCase()} seleccionado`;
  const descParts = [
    platform,
    typeLabel,
    selected.readMode === "always" ? "Leer todos" : "Leer un mensaje",
    selected.voiceName ? `Voz: ${selected.voiceName}` : "Voz automática",
  ];
  if (isGift && selected.triggerCoins) descParts.push(`${selected.triggerCoins} coins`);
  if (els.voicebotSelectionThumb) {
    const image = isGift ? normalizeImageSource(selected.triggerImage) : "";
    els.voicebotSelectionThumb.innerHTML = image
      ? `<img src="${ESC(image)}" alt="${ESC(title)}" onerror="this.style.display='none'">`
      : ESC(selected.triggerEmoji || (isGift ? "🎁" : "✨"));
  }
  if (els.voicebotSelectionTitle) els.voicebotSelectionTitle.textContent = title;
  if (els.voicebotSelectionDesc) els.voicebotSelectionDesc.textContent = descParts.join(" • ");
}

function renderVoicebotPicker() {
  const platform = voicebotRuntime.currentPlatform;
  const triggerType = voicebotRuntime.currentTrigger;
  const library = getVoicebotLibrary(platform, triggerType);
  const search = normalizeVoicebotKey(els.voicebotSearchInput?.value || "");
  const cfg = voicebotSettings();
  const selected = voicebotRuntime.currentDraft || normalizeVoicebotRule({ platform, triggerType });
  let rows = library;

  if (platform === "tiktok" && triggerType === "gift") {
    const items = voicebotRuntime.currentGiftCatalog || [];
    rows = items.map((item) => ({
      id: String(item.id || ""),
      key: item.key || item.name,
      label: item.name || item.alt || "Regalo",
      subtitle: `${Number(item.coins || 0) || 0} coins`,
      emoji: "🎁",
      image: item.image || "",
      coins: Number(item.coins || 0) || 0,
    }));
  } else if (platform === "twitch" && triggerType === "gift") {
    rows = getVoicebotLibrary(platform, triggerType).map((item) => ({
      key: item.key,
      label: item.label,
      subtitle: "Bits",
      emoji: "💎",
  ...
  rows = rows.filter((item) => {
    ...
    return normalizeVoicebotKey(item.label).includes(search) || normalizeVoicebotKey(item.key).includes(search);
  });

  els.voicebotPickerLabel.textContent = `Selecciona ${triggerTypeLabel(platform, triggerType).toLowerCase()}`;
  els.voicebotPickerHint.textContent = platform === "tiktok" && triggerType === "gift"
    ? "Las imágenes se muestran pequeñas para no saturar la pantalla."
    : "Usa la búsqueda para filtrar la lista.";

  els.voicebotPickerList.innerHTML = rows.length ? rows.map((item) => {
    const isSelected = normalizeVoicebotKey(selected.triggerKey) === normalizeVoicebotKey(item.key)
      || normalizeVoicebotKey(selected.triggerId) === normalizeVoicebotKey(item.id);
    const thumb = item.image ? `<img class="voicebotPickerThumb" src="${ESC(item.image)}" alt="${ESC(item.label)}" loading="lazy" onerror="this.style.display='none'">` : `<div class="voicebotPickerThumb" aria-hidden="true" style="display:grid;place-items:center;font-size:15px">${ESC(item.emoji || "•")}</div>`;
    return `
      <button class="voicebotPickerItem ${isSelected ? "selected" : ""}" type="button"
        data-voicebot-pick="${ESC(item.key)}"
        data-voicebot-id="${ESC(item.id || "")}"
        data-voicebot-label="${ESC(item.label)}"
        data-voicebot-image="${ESC(item.image || "")}" 
        data-voicebot-coins="${ESC(item.coins || "")}">
        ${thumb}
        <span class="voicebotPickerText">
    ...
    <div class="emptyState">
      ...
      <span>No hay coincidencias con la búsqueda.</span>
    </div>`;
}

async function loadVoicebotGiftCatalog() {
  if (voicebotRuntime.currentGiftCatalog) return voicebotRuntime.currentGiftCatalog;
  try {
    const res = await fetch("/data/tiktok-gifts.json");
    const data = res.ok ? await res.json() : null;
    const items = Array.isArray(data?.items) ? data.items : Array.isArray(data) ? data : [];
    voicebotRuntime.currentGiftCatalog = items;
    return items;
  } catch {
    voicebotRuntime.currentGiftCatalog = [];
    return [];
  }
}

function updateVoicebotDraftFromUI() {
  const cfg = voicebotSettings();
  if (!voicebotRuntime.currentDraft) {
    voicebotRuntime.currentDraft = normalizeVoicebotRule({
      platform: voicebotRuntime.currentPlatform,
  ...
  voicebotRuntime.currentDraft.platform = voicebotRuntime.currentPlatform;
  voicebotRuntime.currentDraft.triggerType = voicebotRuntime.currentTrigger;
  voicebotRuntime.currentDraft.readMode = els.voicebotReadMode?.value === "always" ? "always" : "once";
  voicebotRuntime.currentDraft.voiceName = els.voicebotVoiceSelect?.value || "";
  voicebotRuntime.currentDraft.sayDiceBeforeMessage = els.voicebotSayDice?.checked !== false;
  voicebotRuntime.currentDraft.ignoreSpecialChars = els.voicebotIgnoreSpecial?.checked !== false;
  voicebotRuntime.currentDraft.ignoreEmotes = els.voicebotIgnoreEmotes?.checked !== false;
  voicebotRuntime.currentDraft.spanishOnly = els.voicebotSpanishOnly?.checked !== false;
  updateVoicebotSelectionSummary(voicebotRuntime.currentDraft);
  return voicebotRuntime.currentDraft;
}

function saveVoicebotRule() {
  const cfg = voicebotSettings();
  const draft = updateVoicebotDraftFromUI();
  if (!draft.triggerKey) {
    toast("Selecciona una opción", "Elige un regalo, evento o rol antes de agregar.", "err");
    return;
  }
  const normalized = normalizeVoicebotRule(draft);
  if (normalized.triggerType === "gift" && !normalized.triggerId && voicebotRuntime.currentGiftCatalog?.length) {
    const fallback = voicebotRuntime.currentGiftCatalog.find((item) => normalizeVoicebotKey(item.key || item.name) === normalizeVoicebotKey(normalized.triggerKey));
    if (fallback) {
      normalized.triggerId = String(fallback.id || "");
      normalized.triggerImage = String(fallback.image || "");
      normalized.triggerCoins = Number(fallback.coins || 0) || 0;
      normalized.triggerLabel = fallback.name || normalized.triggerLabel;
    }
  }
  cfg.rules = cfg.rules.filter((rule) => normalizeVoicebotKey(rule.id) !== normalizeVoicebotKey(normalized.id));
  cfg.rules.push(normalized);
  voicebotRuntime.currentDraft = normalizeVoicebotRule({
    platform: voicebotRuntime.currentPlatform,
    triggerType: voicebotRuntime.currentTrigger,
    readMode: "once",
  ...
  state.settings.personal.voicebot = cfg;
  saveJSON(SETTINGS_KEY, state.settings);
  saveJSON(LEGACY_SETTINGS_KEY, state.settings);
  renderVoicebotModal();
  renderAll();
  toast("Acción agregada", `${triggerButtonLabel(normalized.platform, normalized.triggerType)} guardado.`);
}

function deleteVoicebotRule(ruleId) {
  const cfg = voicebotSettings();
  cfg.rules = cfg.rules.filter((rule) => normalizeVoicebotKey(rule.id) !== normalizeVoicebotKey(ruleId));
  state.settings.personal.voicebot = cfg;
  saveJSON(SETTINGS_KEY, state.settings);
  saveJSON(LEGACY_SETTINGS_KEY, state.settings);
  renderVoicebotModal();
...
function setVoicebotMode(mode) {
  const cfg = voicebotSettings();
  cfg.mode = ["all", "donors", "custom"].includes(mode) ? mode : "custom";
  state.settings.personal.voicebot = cfg;
  saveJSON(SETTINGS_KEY, state.settings);
  saveJSON(LEGACY_SETTINGS_KEY, state.settings);
  renderVoicebotModal();
...
function voicebotUserKey(item) {
  return normalizeVoicebotKey(item?.uniqueId || item?.user || item?.displayName || "");
}

function voicebotActiveGiftName(item) {
  const catalogHit = lookupGiftCatalog(item?.giftId || item?.giftKey || item?.gift || item?.giftName || "");
  return String(item?.gift || item?.giftName || catalogHit?.name || item?.giftAlt || "").trim();
}

function voicebotActiveGiftId(item) {
  return normalizeVoicebotKey(item?.giftId || item?.giftKey || "");
}

function voicebotBadgeMarkup(item) {
  const platform = String(item?.platform || "tiktok").toLowerCase();
  const key = voicebotUserKey(item);
  if (!key) return "";
  const cfg = voicebotSettings();
  const always = voicebotRuntime.activeAlways[platform];
  const once = voicebotRuntime.activeOnce.get(`${platform}:${key}`);
  const donor = voicebotRuntime.donorUsers[platform];
  const active = cfg.mode === "all" || donor.has(key) || always.has(key) || Boolean(once);
  return active ? `<span class="voicebotBadge">Bot voz</span>` : "";
}
...
function stripTwitchEmotes(text, emotes) {
  ...
  if (emotes && typeof emotes === "object") {
    const ranges = [];
    for (const values of Object.values(emotes)) {
    ...
    for (const [start, end] of ranges) {
      result = `${result.slice(0, Math.max(0, start))} ${result.slice(Math.min(result.length, end + 1))}`;
    }
  }
  return result.replace(/\s+/g, " ").trim();
}

function normalizeVoicebotSpeechText(item, rawText = "") {
  let text = String(rawText || "").trim();
  if (!text) return "";
  if (VOICEBOT_STICKER_PATTERNS.test(text)) return "";
  text = text.replace(/\b(?:https?:\/\/\S+|www\.\S+)\b/gi, " ");
  text = text.replace(VOICEBOT_SPECIAL_FILTER, " ");
  text = text.replace(/[^\p{L}\p{N}\s.,!?¿¡'’áéíóúüñÁÉÍÓÚÜÑ-]/gu, " ");
  text = text.replace(/\s+/g, " ").trim();

  if (!text) return "";

  if (/\b(?:kappa|pogchamp|lul|lulw|omegalul|pepe|monkas|dansgame|widepeepo|catjam|frog)\b/i.test(text)) {
    text = text.replace(/\b(?:kappa|pogchamp|lul|lulw|omegalul|pepe|monkas|dansgame|widepeepo|catjam|frog)\b/gi, " ").replace(/\s+/g, " ").trim();
  }

  if (!text) return "";
  if (VOICEBOT_STICKER_PATTERNS.test(text)) return "";
  if (!/[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]/.test(text)) return "";
  return text;
}

function buildVoicebotText(item, rule = null) {
  const cfg = voicebotSettings();
  const effective = rule ? normalizeVoicebotRule(rule) : null;
  const sayDice = effective ? effective.sayDiceBeforeMessage !== false : cfg.sayDiceBeforeMessage !== false;
  const ignoreSpecial = effective ? effective.ignoreSpecialChars !== false : cfg.ignoreSpecialChars !== false;
  const ignoreEmotes = effective ? effective.ignoreEmotes !== false : cfg.ignoreEmotes !== false;
  const spanishOnly = effective ? effective.spanishOnly !== false : cfg.spanishOnly !== false;

  const platform = String(item?.platform || "").toLowerCase();
  const rawMessage = platform === "twitch" ? stripTwitchEmotes(String(item?.message || ""), item?.emotes) : String(item?.message || item?.comment || item?.text || "");
  let text = String(rawMessage || "").trim();

  if (ignoreEmotes) {
    if (platform === "tiktok" && (item?.sticker || item?.stickerImage || item?.stickerAlt || item?.emoteList?.length)) {
      const sticker = normalizeVoicebotSpeechText(item, String(item?.stickerAlt || item?.sticker?.name || item?.sticker?.title || item?.stickerName || ""));
      if (!text || !/[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]/.test(text) || normalizeVoicebotKey(text) === normalizeVoicebotKey(sticker)) {
        return "";
      }
    }
  }
  ...
  if (ignoreSpecial) {
    text = text.replace(VOICEBOT_SPECIAL_FILTER, " ").replace(/[<>^@]/g, " ");
  }

  text = text.replace(/\s+/g, " ").trim();
  if (spanishOnly) {
    text = normalizeVoicebotSpeechText(item, text);
  } else {
    if (VOICEBOT_STICKER_PATTERNS.test(text)) return "";
    text = text.replace(/\s+/g, " ").trim();
  }
  ...
  const name = String(item?.displayName || item?.user || "Usuario").trim();
  const prefix = sayDice ? `${name} dice` : name;
  return `${prefix} ${text}`.replace(/\s+/g, " ").trim();
}

function pickVoicebotVoice(rule = null) {
  const preferred = String(rule?.voiceName || "").trim();
  if (preferred) {
    const hit = voicebotRuntime.voices.find((voice) => voice.name === preferred);
    if (hit) return hit;
  }
  return voicebotRuntime.voices.find((voice) => voice.default) || voicebotRuntime.voices[0] || null;
}

function queueVoicebotSpeech(text, rule = null) {
  const speechText = String(text || "").trim();
  if (!speechText) return;
  voicebotRuntime.speakingQueue.push({ text: speechText, rule: rule ? normalizeVoicebotRule(rule) : null });
  if (!voicebotRuntime.speaking) runVoicebotQueue();
}

async function runVoicebotQueue() {
  if (voicebotRuntime.speaking) return;
  voicebotRuntime.speaking = true;
  while (voicebotRuntime.speakingQueue.length) {
    const next = voicebotRuntime.speakingQueue.shift();
    const utter = new SpeechSynthesisUtterance(next.text);
    const voice = pickVoicebotVoice(next.rule);
  ...
  voicebotRuntime.speaking = false;
}

function findVoicebotRuleForChat(item) {
  const cfg = voicebotSettings();
  const platform = String(item?.platform || "tiktok").toLowerCase();
  const key = voicebotUserKey(item);
  if (!key) return null;
  const exactOnce = voicebotRuntime.activeOnce.get(`${platform}:${key}`);
  if (exactOnce?.rule) return exactOnce.rule;
  if (voicebotRuntime.activeAlways[platform].has(key)) {
    return cfg.rules.map(normalizeVoicebotRule).find((rule) => rule.platform === platform && rule.readMode === "always") || null;
  }
  return cfg.rules.map(normalizeVoicebotRule).find((rule) => rule.platform === platform && rule.readMode === "always") || null;
}

function activateVoicebotFromEvent(item) {
  const cfg = voicebotSettings();
  const platform = String(item?.platform || "tiktok").toLowerCase();
  const key = voicebotUserKey(item);
  if (!key) return;

  const type = normalizeVoicebotKey(item?.type || "");
  const action = normalizeVoicebotKey(item?.action || "");
  const message = normalizeVoicebotKey(item?.message || "");
  const giftName = normalizeVoicebotKey(voicebotActiveGiftName(item));
  const giftId = voicebotActiveGiftId(item);
  const roleKeys = new Set(normalizeBadgeKeys(item.badges).map((value) => normalizeVoicebotKey(value)));
  const candidates = cfg.rules.map(normalizeVoicebotRule).filter((rule) => rule.platform === platform && rule.enabled !== false);

  const donorHit = type.includes("gift") || type.includes("sub") || type.includes("bits") || type.includes("raid") || type.includes("host");
  if (donorHit) voicebotRuntime.donorUsers[platform].add(key);

  for (const rule of candidates) {
    const ruleKey = normalizeVoicebotKey(rule.triggerKey);
    let matched = false;

    if (rule.triggerType === "gift") {
      const ruleId = normalizeVoicebotKey(rule.triggerId || "");
      matched = platform === "tiktok"
        ? (
            (giftId && ruleId && giftId === ruleId) ||
            (giftName && (giftName === ruleKey || normalizeVoicebotKey(rule.triggerLabel) === giftName || message.includes(ruleKey) || action.includes(ruleKey)))
          )
        : (type.includes("bits") && (ruleKey.includes("bits") || ruleKey === "" || message.includes(ruleKey)));
    } else if (rule.triggerType === "event") {
      matched = [type, action, message].some((value) => value.includes(ruleKey)) || (platform === "tiktok" && VOICEBOT_TIKTOK_EVENTS.some((entry) => normalizeVoicebotKey(entry.key) === ruleKey && (type.includes(entry.key) || action.includes(entry.key) || message.includes(entry.key))));
      if (platform === "twitch") matched = matched || VOICEBOT_TWITCH_EVENTS.some((entry) => normalizeVoicebotKey(entry.key) === ruleKey && (type.includes(entry.key) || action.includes(entry.key) || message.includes(entry.key)));
    } else if (rule.triggerType === "role") {
      matched = [...roleKeys].some((value) => value === ruleKey || value.includes(ruleKey) || ruleKey.includes(value));
    }

    if (matched) {
      if (rule.readMode === "always") {
        voicebotRuntime.activeAlways[platform].add(key);
      } else {
        const now = Date.now();
        voicebotRuntime.activeOnce.set(`${platform}:${key}`, { rule, at: now, expiresAt: now + (10 * 60 * 1000) });
      }
    }
  }
}

function maybeSpeakChat(item) {
  const cfg = voicebotSettings();
  const platform = String(item?.platform || "tiktok").toLowerCase();
  const key = voicebotUserKey(item);
  if (!key) return;
  const pending = voicebotRuntime.activeOnce.get(`${platform}:${key}`);
  const always = voicebotRuntime.activeAlways[platform].has(key);
  const donor = voicebotRuntime.donorUsers[platform].has(key);

  if (pending && pending.expiresAt && pending.expiresAt < Date.now()) {
    voicebotRuntime.activeOnce.delete(`${platform}:${key}`);
  }

  const livePending = voicebotRuntime.activeOnce.get(`${platform}:${key}`);
  const rule = livePending?.rule || (always ? cfg.rules.map(normalizeVoicebotRule).find((entry) => entry.platform === platform && entry.readMode === "always") : null);

  const shouldSpeak = cfg.mode === "all" || donor || always || Boolean(livePending) || Boolean(rule);
  if (!shouldSpeak) return;

  if (cfg.mode === "donors" && !donor && !always && !livePending) return;
  if (cfg.mode === "custom" && !livePending && !always) return;

  const text = buildVoicebotText(item, rule || pending?.rule || null);
  if (!text) return;

  queueVoicebotSpeech(text, rule || pending?.rule || null);
  if (pending) voicebotRuntime.activeOnce.delete(`${platform}:${key}`);
}

async function initVoicebot() {
  await loadVoicebotGiftCatalog();
  refreshVoicebotVoices();
  if (window.speechSynthesis) {
    window.speechSynthesis.onvoiceschanged = refreshVoicebotVoices;
  }

  els.openVoicebotBtn?.addEventListener("click", openVoicebotModal);
  els.closeVoicebotBtn?.addEventListener("click", closeVoicebotModal);
  els.resetVoicebotBtn?.addEventListener("click", () => {
    state.settings.personal.voicebot = structuredClone(defaults.personal.voicebot);
    saveJSON(SETTINGS_KEY, state.settings);
    saveJSON(LEGACY_SETTINGS_KEY, state.settings);
    loadSettingsToUI();
    renderAll();
    renderVoicebotModal();
    toast("Bot de voz restaurado", "Se recuperaron los ajustes base.");
  });
  els.saveVoicebotBtn?.addEventListener("click", () => {
    saveJSON(SETTINGS_KEY, state.settings);
    saveJSON(LEGACY_SETTINGS_KEY, state.settings);
    toast("Bot de voz guardado", "Las reglas quedaron listas.");
    closeVoicebotModal();
  });
  els.voicebotSaveRuleBtn?.addEventListener("click", saveVoicebotRule);
  els.voicebotSearchInput?.addEventListener("input", renderVoicebotPicker);
  els.voicebotVoiceSelect?.addEventListener("change", () => {
    updateVoicebotDraftFromUI();
    updateVoicebotSelectionSummary();
  });
  els.voicebotReadMode?.addEventListener("change", () => {
    updateVoicebotDraftFromUI();
    updateVoicebotSelectionSummary();
  });

  els.voicebotSayDice?.addEventListener("change", () => {
    updateVoicebotDraftFromUI();
    updateVoicebotSelectionSummary();
  });
  els.voicebotIgnoreSpecial?.addEventListener("change", () => {
    updateVoicebotDraftFromUI();
    updateVoicebotSelectionSummary();
  });
  els.voicebotIgnoreEmotes?.addEventListener("change", () => {
    updateVoicebotDraftFromUI();
    updateVoicebotSelectionSummary();
  });
  els.voicebotSpanishOnly?.addEventListener("change", () => {
    updateVoicebotDraftFromUI();
    updateVoicebotSelectionSummary();
  });

  document.addEventListener("click", (event) => {
    const platformBtn = event.target.closest?.("[data-voicebot-platform]");
    const triggerBtn = event.target.closest?.("[data-voicebot-trigger]");
    const pickBtn = event.target.closest?.("[data-voicebot-pick]");
    const deleteBtn = event.target.closest?.("[data-voicebot-delete]");
    const modeBtn = event.target.closest?.("[data-voicebot-mode]");

    if (platformBtn) {
      voicebotRuntime.currentPlatform = platformBtn.dataset.voicebotPlatform || "tiktok";
      voicebotRuntime.currentDraft = normalizeVoicebotRule({
        platform: voicebotRuntime.currentPlatform,
        triggerType: voicebotRuntime.currentTrigger,
      });
    ...
    if (triggerBtn) {
      voicebotRuntime.currentTrigger = triggerBtn.dataset.voicebotTrigger || "gift";
      voicebotRuntime.currentDraft = normalizeVoicebotRule({
        platform: voicebotRuntime.currentPlatform,
        triggerType: voicebotRuntime.currentTrigger,
      });
    ...
    if (modeBtn) {
      setVoicebotMode(modeBtn.dataset.voicebotMode || "custom");
      return;
    }
    if (pickBtn) {
      const draft = updateVoicebotDraftFromUI();
      draft.triggerKey = pickBtn.dataset.voicebotPick || "";
      draft.triggerId = pickBtn.dataset.voicebotId || draft.triggerId || "";
      draft.triggerLabel = pickBtn.dataset.voicebotLabel || draft.triggerKey || "";
      draft.triggerImage = pickBtn.dataset.voicebotImage || draft.triggerImage || "";
      draft.triggerCoins = Number(pickBtn.dataset.voicebotCoins || draft.triggerCoins || 0) || 0;
      draft.triggerEmoji = pickBtn.querySelector(".voicebotPickerThumb")?.textContent?.trim() || (voicebotRuntime.currentPlatform === "twitch" && voicebotRuntime.currentTrigger === "gift" ? "💎" : "🎁");
      voicebotRuntime.currentDraft = draft;
      renderVoicebotPicker();
      updateVoicebotSelectionSummary(draft);
      return;
    }
    if (deleteBtn) {
      deleteVoicebotRule(deleteBtn.dataset.voicebotDelete || "");
      return;
    }
  });

  await ensureGiftCatalog();
  voicebotRuntime.currentGiftCatalog = giftCatalog;
  renderVoicebotModal();
}
...
socket.on("event", (data) => {
  ...
  activateVoicebotFromEvent(item);
});

bootstrap();

initVoicebot().catch((error) => console.error("Voicebot init failed:", error));
