(() => {
  ...
  function renderCustomize() {
    const p=settings.personalization;
    $('view').innerHTML=`<div class="intro"><h2>Personalización</h2><p>Cada pestaña controla únicamente el diseño interno del dashboard. Los overlays mantienen su propia apariencia y solo comparten la conexión/eventos.</p></div><div class="tabs"><button class="tab ${activeCustomizeTab==='chat'?'active':''}" data-tab="chat">Chat</button><button class="tab ${activeCustomizeTab==='events'?'active':''}" data-tab="events">Eventos</button><button class="tab ${activeCustomizeTab==='gifts'?'active':''}" data-tab="gifts">Regalos</button></div><div id="customArea"></div>`;
    const draw=tab=>{
      activeCustomizeTab=tab;
      if(tab==='chat'){
        $('customArea').innerHTML=`<div class="settings-grid three custom-layout"><article class="card"><p class="eyebrow">ESTRUCTURA</p><h3>Chat del dashboard</h3>${ctl('Tema del chat','cTheme','select',p.chatTheme,'<option value="cloud">Cloud</option><option value="glass">Glass</option><option value="bubble">Bubble</option><option value="neon">Neon</option><option value="minimal">Minimal</option><option value="aurora">Aurora</option><option value="comic">Comic</option><option value="holo">Holo</option><option value="ribbon">Ribbon</option>')}${ctl('Tipo de letra','cFont','select',p.font||'inter','<option value="inter">Inter / Manrope</option><option value="poppins">Poppins</option><option value="montserrat">Montserrat</option><option value="oswald">Oswald</option><option value="system">Sistema</option>')}${ctl('Distribución','cLayout','select',p.chatLayout,'<option value="vertical">Vertical</option><option value="horizontal">Horizontal</option>')}${ctl('Dirección','cDirection','select',p.chatDirection,'<option value="down">Más reciente abajo</option><option value="up">Más reciente arriba</option>')}${ctl('Animación','cAnim','select',p.animation,'<option value="slide">Slide</option><option value="fade">Fade</option><option value="pop">Pop</option><option value="none">Sin animación</option>')}${ctl('Modo horizontal','cHorizontal','select',p.chatHorizontalMode,'<option value="compact">Compacto</option><option value="normal">Normal</option><option value="wide">Alargado</option>')}${ctl('Forma interna','cShape','select',p.chatOverlayShape,'<option value="normal">Normal</option><option value="card">Tarjeta</option>')}${ctl('Ajustar mensajes largos','cAdjust','check',p.chatAdjustMessages)}</article><article class="card"><p class="eyebrow">IDENTIDAD</p><h3>Avatar, nombre e insignias</h3>${ctl('Marco avatar','cAvatar','select',p.avatarFrame,'<option value="platform">Plataforma</option><option value="ring">Anillo</option><option value="role">Rol</option><option value="none">Sin marco</option>')}${ctl('Tamaño avatar','cAvatarSize','select',p.avatarSize,'<option value="sm">Pequeño</option><option value="md">Medio</option><option value="lg">Grande</option>')}${ctl('Tamaño nombre','cNameSize','select',p.nameSize,'<option value="sm">Pequeño</option><option value="md">Medio</option><option value="lg">Grande</option>')}${ctl('Peso nombre','cNameWeight','select',p.nameWeight,'<option value="600">Semibold</option><option value="700">Bold</option><option value="800">Extra Bold</option><option value="900">Black</option>')}${ctl('Marco comentario','cBubble','select',p.bubbleFrame,'<option value="platform">Plataforma</option><option value="role">Rol</option><option value="none">Sin marco</option>')}${ctl('Estilo insignias','cBadgeStyle','select',p.badgeStyle,'<option value="emoji">Emojis</option><option value="compact">Compacto</option>')}${ctl('Mostrar insignias','cBadges','check',p.showBadges!==false)}${ctl('Mostrar emotes','cEmotes','check',p.showEmotes!==false)}</article><article class="card"><div class="preview-header"><div><p class="eyebrow">VISTA PREVIA EN TIEMPO REAL</p><h3>Chat del dashboard</h3></div><span class="preview-live"><i></i> LIVE</span></div><div id="liveChatPreview" class="live-chat-preview layout-${esc(p.chatLayout||'vertical')} direction-${esc(p.chatDirection||'down')}">${chatPreviewHtml()}</div><p class="preview-note">Usa los mismos eventos que recibe el dashboard. Los cambios visuales se reflejan sin detener la conexión.</p></article><article class="card"><p class="eyebrow">COLORES</p><h3>Identidad por plataforma</h3>${ctl('TikTok','cTikTokName','select',p.tiktokNameColor,'<option value="white">Blanco</option><option value="pink">Rosado</option><option value="platform">Color plataforma</option>')}${ctl('Twitch','cTwitchName','select',p.twitchNameColor,'<option value="real">Color de Twitch</option><option value="platform">Color plataforma</option><option value="white">Blanco</option>')}${ctl('Color del mensaje','cTextColor','select',p.textColor,'<option value="auto">Automático</option><option value="#ffffff">Blanco</option><option value="#d9d9e4">Gris claro</option><option value="#ffd76e">Dorado</option><option value="#9fe8ff">Celeste</option>')}</article><article class="card"><p class="eyebrow">EFECTOS</p><h3>Texto</h3>${ctl('Efecto mensaje','cMessageEffect','select',p.messageEffect,'<option value="none">Sin efecto</option><option value="shadow">Sombra suave</option><option value="outline">Contorno</option>')}${ctl('Efecto nombre','cNameEffect','select',p.nameEffect,'<option value="none">Sin efecto</option><option value="shadow">Sombra suave</option><option value="outline">Contorno</option>')}</article><article class="card"><p class="eyebrow">SUPPORTERS</p><h3>Resaltados</h3>${ctl('Resaltar donadores','cSupporterTikTok','check',p.highlightSupporters!==false)}${ctl('Estilo destacado','cSupporterStyle','select',p.supporterHighlightStyle,'<option value="gold">Dorado</option><option value="platform">Plataforma</option>')}${ctl('Auto limpiar chat','cAutoClear','check',p.autoClearChat)}${ctl('Segundos','cClearSeconds','input',p.clearChatSeconds||30)}</article></div>`;
      } else {
        const events=tab==='events'; const layout=events?p.eventsLayout:p.giftsLayout, direction=events?p.eventsDirection:p.giftsDirection, mode=events?p.eventsMode:p.giftsMode, size=events?p.eventsPanelSize:p.giftsPanelSize;
        const keys=events?['highlightLikes','highlightFollows','highlightJoins','highlightShares','highlightSystem','highlightFanclub','highlightSuperfan']:['highlightGifts','highlightSubs','highlightBits','highlightRaids'];
        $('customArea').innerHTML=`<div class="settings-grid three custom-layout"><article class="card"><p class="eyebrow">${events?'EVENTOS':'REGALOS'}</p><h3>Diseño interno</h3>${ctl('Distribución',events?'eLayout':'gLayout','select',layout,'<option value="vertical">Vertical</option><option value="horizontal">Horizontal</option>')}${ctl('Dirección',events?'eDirection':'gDirection','select',direction,'<option value="down">Abajo</option><option value="up">Arriba</option><option value="left">Izquierda</option><option value="right">Derecha</option>')}${ctl('Animación',events?'eMode':'gMode','select',mode,'<option value="slide">Slide</option><option value="fade">Fade</option><option value="stack">Stack</option>')}${ctl('Tamaño',events?'eSize':'gSize','select',size,'<option value="compact">Compacto</option><option value="normal">Normal</option><option value="large">Grande</option><option value="xl">XL</option>')}${ctl('Marco de tarjeta',events?'eFrame':'gFrame','check',events?p.eventsCardFrame!==false:p.giftsCardFrame!==false)}${!events?ctl('Composición regalo','gComposition','select',p.overlayGiftComposition,'<option value="normal">Normal</option><option value="elongated">Alargada</option>'):''}${!events?ctl('Tamaño imagen regalo','giftImg','select',p.overlayGiftImageSize,'<option value="sm">Pequeña</option><option value="md">Mediana</option><option value="lg">Grande</option>'):''}</article><article class="card"><p class="eyebrow">DESTACADOS</p><h3>Qué se resalta</h3>${keys.map(k=>ctl(k.replace('highlight','Mostrar '),'x_'+k,'check',p[k]!==false)).join('')}${ctl('Usuario destacado','x_highlightEventUsername','check',p.highlightEventUsername!==false)}${events?ctl('Color destacado','eHighlightStyle','select',p.overlayEventHighlightStyle,'<option value="platform">Plataforma</option><option value="gold">Dorado</option><option value="rainbow">Variado</option>'):ctl('Color regalo','gHighlightStyle','select',p.giftHighlightStyle,'<option value="gold">Dorado</option><option value="platform">Plataforma</option><option value="rainbow">Variado</option>')}</article><article class="card preview-card"><p class="eyebrow">VISTA PREVIA</p><div class="preview-stack">${messageRow(events?{platform:'twitch',displayName:'Nuevo seguidor',username:'seguidor',type:'follow',message:'Empezó a seguirte'}:{platform:'tiktok',displayName:'Supporter',username:'supporter',type:'gift',gift:'Rose',amount:1},events?'event':'gift')}</div><p class="preview-note">Estos cambios son exclusivos del dashboard. El overlay conserva su propio diseño.</p></article></div>`;
      }
      const pairs={cTheme:['personalization','chatTheme'],cFont:['personalization','font'],cAvatar:['personalization','avatarFrame'],cAvatarSize:['personalization','avatarSize'],cNameSize:['personalization','nameSize'],cNameWeight:['personalization','nameWeight'],cBubble:['personalization','bubbleFrame'],cBadgeStyle:['personalization','badgeStyle'],cBadges:['personalization','showBadges'],cEmotes:['personalization','showEmotes'],cLayout:['personalization','chatLayout'],cDirection:['personalization','chatDirection'],cAnim:['personalization','animation'],cHorizontal:['personalization','chatHorizontalMode'],cShape:['personalization','chatOverlayShape'],cAdjust:['personalization','chatAdjustMessages'],cTikTokName:['personalization','tiktokNameColor'],cTwitchName:['personalization','twitchNameColor'],cTextColor:['personalization','textColor'],cMessageEffect:['personalization','messageEffect'],cNameEffect:['personalization','nameEffect'],cSupporterTikTok:['personalization','highlightSupporters'],cSupporterStyle:['personalization','supporterHighlightStyle'],cAutoClear:['personalization','autoClearChat'],cClearSeconds:['personalization','clearChatSeconds'],eLayout:['personalization','eventsLayout'],eDirection:['personalization','eventsDirection'],eMode:['personalization','eventsMode'],eSize:['personalization','eventsPanelSize'],eFrame:['personalization','eventsCardFrame'],gLayout:['personalization','giftsLayout'],gDirection:['personalization','giftsDirection'],gMode:['personalization','giftsMode'],gSize:['personalization','giftsPanelSize'],gFrame:['personalization','giftsCardFrame'],gComposition:['personalization','overlayGiftComposition'],giftImg:['personalization','overlayGiftImageSize'],eHighlightStyle:['personalization','overlayEventHighlightStyle'],gHighlightStyle:['personalization','giftHighlightStyle']};
      document.querySelectorAll('#customArea select,#customArea input').forEach(el=>el.addEventListener('change',async()=>{const key=pairs[el.id]||(el.id.startsWith('x_')?['personalization',el.id.slice(2)]:null);if(!key)return;let value=el.type==='checkbox'?el.checked:el.value;if(el.type==='number')value=Number(value);if(el.id==='cClearSeconds')value=Number(value||30);const patch={[key[0]]:{[key[1]]:value}};settings=merge(settings,patch);applyAppearance();if(activeCustomizeTab==='chat')renderCustomizePreviewOnly();else draw(activeCustomizeTab);await persistSettingsPatch(patch,false);}));
      const selectMap=['cTheme','cFont','cAvatar','cAvatarSize','cNameSize','cNameWeight','cBubble','cBadgeStyle','cLayout','cDirection','cAnim','cHorizontal','cShape','cTikTokName','cTwitchName','cTextColor','cMessageEffect','cNameEffect','cSupporterStyle','eLayout','eDirection','eMode','eSize','gLayout','gDirection','gMode','gSize','gComposition','giftImg','eHighlightStyle','gHighlightStyle']; selectMap.forEach(id=>{const el=$(id);if(!el)return;const k=pairs[id]?.[1];if(k)setSelect(id,p[k]);});
      ['cBadges','cEmotes','cAdjust','cSupporterTikTok','cAutoClear','eFrame','gFrame'].forEach(id=>{const el=$(id);if(!el)return;const k=pairs[id]?.[1];if(k)setCheck(id,p[k]!==false);});
  ...
  function overlayCard(name, path, description) {
    const url = `${location.origin}/${path}${path.includes('?')?'&':'?'}owner=${encodeURIComponent(user.id)}`;
    return `<article class="card overlay-card"><div class="mini-preview">${name==='Ruleta'?'🎡':name==='Lista de voces'?'🎙️':name==='Chat'?'💬':'✨'}</div><p class="eyebrow">SALIDA OBS</p><h3>${esc(name)}</h3><p class="muted">${esc(description)}</p><code>${esc(url)}</code><div class="row"><button class="btn primary openPopup" data-url="${esc(url)}">Abrir ventana</button><a class="btn secondary" href="${esc(url)}" target="_blank" rel="noopener">Pestaña</a><button class="btn secondary copyLink" data-url="${esc(url)}">Copiar</button></div></article>`;
  }

  function renderOverlays() {
    $('view').innerHTML=`<div class="intro"><h2>Overlays</h2><p>Son salidas independientes para OBS. Solo comparten la conexión del usuario y la fuente de eventos; su diseño no se copia del dashboard.</p></div><div class="overlay-status"><span class="status-pill ${state.connection==='online'?'on':''}"><i></i>${state.connection==='online'?'Conectado al stream':'Sin conexión'}</span>${['tiktok','twitch'].map(p=>`<span class="channel-state ${isConnected(p)?'on':''}">${p==='tiktok'?'TikTok':'Twitch'} · ${isConnected(p)?'ON':'OFF'}</span>`).join('')}</div><div class="overlay-grid">${overlayCard('Chat','overlay.html','Chat overlay independiente; usa la conexión real.')}${overlayCard('Eventos','overlay.html?view=events','Eventos overlay independiente.')}${overlayCard('Regalos','overlay.html?view=gifts','Regalos overlay independiente, con imagen del regalo.')}${overlayCard('Ruleta','roulette-overlay.html','Ruleta overlay original.')}${overlayCard('Lista de voces','voice-list-overlay.html','Widget de lista de voces para OBS.')}</div>`;
    document.querySelectorAll('.openPopup').forEach(b=>b.onclick=()=>openOverlay(b.dataset.url,`sf_${b.dataset.url.split('/').pop()}`));
    document.querySelectorAll('.copyLink').forEach(b=>b.onclick=()=>navigator.clipboard?.writeText(b.dataset.url).then(()=>toast('Enlace copiado')));
  }

  let rouletteState={participants:[],spinning:false};
  function renderRoulette(){
    const url=`${location.origin}/roulette-overlay.html?owner=${encodeURIComponent(user.id)}&embed=1`;
    $('view').innerHTML=`<div class="intro split"><div><h2>Ruleta</h2><p>Esta pestaña muestra la misma ruleta de cartas que utiliza el overlay. No se crea una segunda ruleta ni se altera el motor que ya funciona.</p></div><div class="row"><button class="btn secondary" id="openRoulettePopup">Abrir overlay</button><a class="btn secondary" href="${esc(url)}" target="_blank" rel="noopener">Abrir pestaña</a></div></div><section class="card roulette-overlay-host"><iframe id="rouletteFrame" src="${esc(url)}" title="Ruleta StreamFusion"></iframe></section>`;
    $('openRoulettePopup').onclick=()=>openOverlay(url,'streamfusionRoulette');
  }
  ...
  async function saveVoice(v){
    ...
    try { const data=await api('/api/user/voices',{method:'POST',body:JSON.stringify({fishId,label:label||fishId,tags})}); toast('Voz guardada',`${data.voice?.label||label||fishId} quedó en tu biblioteca.`); await renderVoices(); }
  ...
  function buildVoicePreviewHtml(s){
    const list=voicePreviewItems(); const motion=s.motion||'static'; const vertical=(s.direction||'vertical')==='vertical';
  ...
  function renderWidgets(){
    ...
    const map={vEnabled:['enabled','check'],vTransparent:['transparent','check'],vBgOpacity:['backgroundOpacity','num'],vFont:['fontFamily'],vSize:['fontSize','num'],vWeight:['fontWeight','num'],vStyle:['fontStyle'],vColor:['textColor'],vShadow:['textShadow'],vShadowColor:['shadowColor'],vOutline:['outlineWidth','num'],vOutlineColor:['outlineColor'],vTransform:['textTransform'],vLetter:['letterSpacing','num'],vLine:['lineHeight','num'],vGap:['itemGap','num'],vAlign:['align'],vPosition:['listPosition'],vDirection:['direction'],vMotion:['motion'],vMotionSpeed:['motionSpeed','num'],vShowIndex:['showIndex','check'],vShowId:['showId','check'],vAutoShow:['autoShowEnabled','check'],vAutoEvery:['autoShowEvery','num'],vAutoFor:['autoShowFor','num'],vSelected:['selectedVoice']};
    ...
    setSelect('vFont',s.fontFamily); setSelect('vPosition',s.listPosition); setSelect('vSelected',s.selectedVoice); if($('vColor'))$('vColor').value=s.textColor; if($('vShadowColor'))$('vShadowColor').value=s.shadowColor; if($('vOutlineColor'))$('vOutlineColor').value=s.outlineColor;
    ...
    $('openVoiceWidget').onclick=()=>openOverlay(`${location.origin}/voice-list-overlay.html?owner=${encodeURIComponent(user.id)}`,'streamfusionVoiceList');
