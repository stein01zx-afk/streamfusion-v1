(() => {
  ...
  function renderWidgets(){
    const s=structuredClone(settings.voiceList||{}); s.offsetX=Number(s.offsetX||0); s.offsetY=Number(s.offsetY||0); s.roulette={enabled:false,title:'¿Quieres una voz?',subtitle:'Para participar, comenta lo que se indique en el sorteo!',winnerText:'Si ganas, solo comenta una de las siguientes voces:',titleSeconds:3,subtitleSeconds:3,winnerSeconds:3,introMotion:'fade',cardOpacity:.12,showListAfterIntro:true,...(s.roulette||{})};
    ...
    $('view').innerHTML=`<div class="intro"><h2>Widgets</h2><p>Solo herramientas visuales externas. Por ahora está disponible la Lista de Voces, con el mismo concepto de edición que tenía la versión original.</p></div><div class="widget-editor-tabs"><button class="tab active">Lista de voces</button></div><div class="widget-editor-layout"><section class="card widget-controls"><div class="section-head"><div><p class="eyebrow">EDITOR</p><h3>Lista de Voces</h3></div><div class="row"><button class="btn secondary" id="openVoiceWidget">Abrir overlay</button><button class="btn primary" id="saveVoiceWidget">Guardar</button></div></div><div class="settings-grid two compact-grid">
      ...
      <article class="widget-subsection"><p class="eyebrow">COMPOSICIÓN</p>${voiceCtl('Separación','vGap','input',s.itemGap)}${voiceCtl('Alineación','vAlign','select',s.align,'<option value="left">Izquierda</option><option value="center">Centro</option><option value="right">Derecha</option>')}${voiceCtl('Desplazamiento horizontal','vOffsetX','input',s.offsetX||0)}${voiceCtl('Desplazamiento vertical','vOffsetY','input',s.offsetY||0)}${voiceCtl('Dirección','vDirection','select',s.direction,'<option value="vertical">Vertical</option><option value="horizontal">Horizontal</option>')}${voiceCtl('Movimiento','vMotion','select',s.motion,'<option value="static">Estático</option><option value="scroll">Scroll</option><option value="slide">Slide</option><option value="marquee">Marquee</option><option value="float">Float</option><option value="crawl">Crawl</option><option value="starwars">Star Wars</option>')}${voiceCtl('Velocidad','vMotionSpeed','input',s.motionSpeed)}</article>
  ...
  async function renderSettings(){ const a=settings.appearance||{}; const mods=Array.isArray(settings.tiktokModerators)?settings.tiktokModerators:[]; $('view').innerHTML=`<div class="intro"><h2>Ajustes</h2><p>Todo lo que se guarda aquí pertenece a tu cuenta.</p></div><div class="settings-grid two"><article class="card"><p class="eyebrow">APARIENCIA DEL DASHBOARD</p><h3>Tema</h3>${ctl('Tema','sTheme','select',a.theme||'dark','<option value="dark">Noche profunda</option><option value="midnight">Medianoche</option><option value="light">Claro</option>')}<label>Color de acento<input id="sAccent" type="color" value="${esc(a.accent||'#7c5cff')}"></label><button class="btn primary" id="saveAppearance">Guardar</button></article><article class="card"><p class="eyebrow">MODERADORES TIKTOK</p><h3>Usuarios con 🛡️</h3><p class="muted">Añade el Unique ID exacto de cada moderador del canal. El bot de voz usará esta misma insignia para “Solo moderadores”.</p><textarea id="tiktokModerators" rows="7" style="width:100%;resize:vertical;background:var(--surface2);color:var(--text);border:1px solid var(--line);border-radius:11px;padding:12px;font:500 13px Manrope" placeholder="unqiue_id_1
unique_id_2">${esc(mods.join('\n'))}</textarea><button class="btn primary" id="saveModerators">Guardar moderadores</button></article><article class="card"><p class="eyebrow">CUENTA</p><h3>${esc(user?.displayName||'Creador')}</h3><p>${esc(user?.email||'')}</p><p class="muted">ID: ${esc(user?.id||'')}</p><button class="btn secondary" id="logout2">Cerrar sesión</button></article></div>`; $('saveAppearance').onclick=()=>persistSettingsPatch({appearance:{theme:$('sTheme').value,accent:$('sAccent').value}}); $('saveModerators').onclick=async()=>{const moderators=$('tiktokModerators').value.split(/\n|,/).map(x=>x.trim()).filter(Boolean);try{const data=await api('/api/tiktok/moderators',{method:'PUT',body:JSON.stringify({moderators})});settings.tiktokModerators=data.moderators||moderators;toast('Moderadores guardados',`${settings.tiktokModerators.length} usuarios con 🛡️`);}catch(e){toast('No se guardó',e.message,'err')}}; $('logout2').onclick=logout; }
