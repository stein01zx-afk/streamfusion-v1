(() => {
  ...
  function setupSocket(){
    ...
    socket.on('settings', s=>{
      ...
      settings=incoming;
      const savedProfiles=incoming.connectionProfiles||{};
      for(const p of ['tiktok','twitch']){ const prof=savedProfiles[p]||{}; if(prof.username){ state.accounts[p]={...(state.accounts[p]||{}), username:prof.username, avatarUrl:prof.avatarUrl||'', connected:Boolean(state.accounts[p]?.connected), live:Boolean(state.accounts[p]?.live), mode:state.accounts[p]?.mode||'saved'}; } }
      renderTop();
      applyAppearance();
      if(page==='dashboard') updateDashboardFeeds();
