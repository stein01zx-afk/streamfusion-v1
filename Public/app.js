(() => {
  ...
  function activityBadgeMarkup(item) {
    ...
    if (previewType === 'follow' && settings.personalization.highlightFollows !== false) badges.push('<span class="activity-badge" title="Comenzó a seguir">👤</span>');
  ...
  function normalizeIncomingActivity(item) {
    ...
    if (type === 'follow') { entry.action = 'Comenzó a seguir'; entry.emoji = '👤'; }
    ...
    if (!entry.message && type === 'follow') entry.message = `${displayNameForActivity(entry)} comenzó a seguir`;
    if (!entry.message && type === 'share') entry.message = `${displayNameForActivity(entry)} compartió el LIVE`;
    if (!entry.message && type === 'join') entry.message = `${displayNameForActivity(entry)} se unió al directo`;
    return entry;
  }
