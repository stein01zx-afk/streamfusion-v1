(() => {
    ...
    return `<article class="stream-row ${kind} ${platform} ${isSupporter(item) ? 'supporter-gold' : ''}" style="${styleVars(item)}">
      ...
      <div class="row-body">
        <div class="row-top">
          ...
          ${badgeMarkup(item.isModerator ? [...(Array.isArray(item.badges)?item.badges:[]),'moderator'] : item.badges)}${activityBadgeMarkup(item)}
  ...
  function renderWidgets(){
    ...
    s.roulette={enabled:false,title:'¿Quieres una voz?',subtitle:'Para participar, comenta lo que se indique en el sorteo!',winnerText:'Si ganas, solo comenta una de las siguientes voces:',titleSeconds:3,subtitleSeconds:3,winnerSeconds:3,introMotion:'fade',cardOpacity:.12,showListAfterIntro:true,...(s.roulette||{})};
    s.offsetHorizontal = Number(s.offsetHorizontal ?? 0);
    s.offsetVertical = Number(s.offsetVertical ?? 0);
    const fontOpts=VOICE_FONTS.map(x=>`<option value="${esc(x[0])}">${esc(x[1])}</option>`).join('');
    if(!voiceListEditorOpen){
      $('view').innerHTML=`<div class="intro"><h2>Widgets</h2><p>Configura cada widget por separado. La Lista de Voces solo abre su editor cuando la seleccionas.</p></div>
        <div class="widget-editor-tabs"><button class="tab active" id="openVoiceListEditorBtn">🎙️ Lista de voces</button></div>
        <section class="card widget-launch-card"><div class="section-head"><div><p class="eyebrow">WIDGET</p><h3>Lista de Voces</h3><p class="muted">Abre el editor para personalizar el estilo y ver una vista previa real de la lista.</p></div><button class="btn primary" id="openVoiceListEditorBtn2">Personalizar</button></div></section>`;
      $('openVoiceListEditorBtn').onclick=()=>{voiceListEditorOpen=true;renderWidgets();};
      $('openVoiceListEditorBtn2').onclick=()=>{voiceListEditorOpen=true;renderWidgets();};
      return;
    }

    $('view').innerHTML=`<div class="intro"><div class="intro split"><div><h2>Widgets · Lista de Voces</h2><p>La vista previa usa la misma configuración que recibe el overlay.</p></div><div class="row"><button class="btn secondary" id="closeVoiceListEditorBtn">← Widgets</button><button class="btn secondary" id="openVoiceWidget">Abrir overlay</button><button class="btn primary" id="saveVoiceWidget">Guardar cambios</button></div></div></div>
      <div class="widget-editor-layout">
      <section class="card widget-controls"><div class="section-head"><div><p class="eyebrow">EDITOR</p><h3>Lista de Voces</h3></div></div><div class="settings-grid two compact-grid">
      ...
      <article class="widget-subsection"><p class="eyebrow">COMPOSICIÓN</p>${voiceCtl('Separación','vGap','input',s.itemGap)}${voiceCtl('Alineación','vAlign','select',s.align,'<option value="left">Izquierda</option><option value="center">Centro</option><option value="right">Derecha</option>')}
      <label><span>Desplazamiento horizontal</span><input id="vOffsetX" type="number" step="1" value="${esc(s.offsetHorizontal)}"></label>
      <label><span>Desplazamiento vertical</span><input id="vOffsetY" type="number" step="1" value="${esc(s.offsetVertical)}"></label>
      ${voiceCtl('Dirección','vDirection','select',s.direction,'<option value="vertical">Vertical · arriba → abajo</option><option value="horizontal">Horizontal · izquierda → derecha</option>')}${voiceCtl('Movimiento','vMotion','select',s.motion,'<option value="static">Estático</option><option value="scroll">Scroll</option><option value="slide">Slide</option><option value="marquee">Marquee</option>')}${voiceCtl('Velocidad','vMotionSpeed','input',s.motionSpeed)}</article>
      ...
      <section class="card widget-preview-card"><div class="preview-header"><div><p class="eyebrow">VISTA PREVIA EN TIEMPO REAL</p><h3>Lista de Voces</h3></div><span class="preview-live"><i></i> LIVE</span></div><div id="voiceWidgetPreview" class="voice-widget-preview">${buildVoicePreviewHtml(s)}</div><div class="widget-preview-footer"><span class="muted">Los cambios se reflejan aquí inmediatamente y se guardan al pulsar Guardar cambios.</span><code>/voice-list-overlay.html?owner=${esc(user.id)}</code></div></section></div>`;
    const map={vEnabled:['enabled','check'],vTransparent:['transparent','check'],vBgOpacity:['backgroundOpacity','num'],vFont:['fontFamily'],vSize:['fontSize','num'],vWeight:['fontWeight','num'],vStyle:['fontStyle'],vColor:['textColor'],vShadow:['textShadow'],vShadowColor:['shadowColor'],vOutline:['outlineWidth','num'],vOutlineColor:['outlineColor'],vTransform:['textTransform'],vLetter:['letterSpacing','num'],vLine:['lineHeight','num'],vGap:['itemGap','num'],vAlign:['align'],vDirection:['direction'],vMotion:['motion'],vMotionSpeed:['motionSpeed','num'],vShowIndex:['showIndex','check'],vShowId:['showId','check'],vAutoShow:['autoShowEnabled','check'],vAutoEvery:['autoShowEvery','num'],vAutoFor:['autoShowFor','num'],vSelected:['selectedVoice']};
    ...
    for(const [id,key] of [['vOffsetX','offsetHorizontal'],['vOffsetY','offsetVertical']]){const el=$(id);el?.addEventListener('input',()=>{s[key]=Number(el.value||0);updatePreview();});}
    setSelect('vFont',s.fontFamily); setSelect('vSelected',s.selectedVoice);
    ...
    $('closeVoiceListEditorBtn').onclick=()=>{voiceListEditorOpen=false;renderWidgets();};
  ...
  function renderSettings(){
    ...
    $('view').innerHTML=`<div class="intro"><h2>Ajustes</h2><p>Todo lo que se guarda aquí pertenece a tu cuenta.</p></div>
      ...
      <article class="card moderator-settings-card" style="margin-top:18px">
        <div class="section-head"><div><p class="eyebrow">MODERACIÓN</p><h3>Añadir moderadores TikTok</h3><p class="muted">Agrega el <strong>uniqueId</strong> exacto de cada moderador. Recibirán automáticamente la insignia 🛡️ y entrarán en “Solo moderadores” del bot de voz.</p></div><span class="count-pill">${mods.length} configurados</span></div>
        <div class="row" style="margin:14px 0"><input id="tiktokModeratorInput" placeholder="uniqueId de TikTok" autocomplete="off" style="flex:1;min-width:220px"><button class="btn primary" id="addTiktokModerator">Añadir moderador</button></div>
        <div id="tiktokModeratorList" class="moderator-list">${mods.length?mods.map((m,i)=>`<div class="moderator-row"><span class="moderator-badge">🛡️</span><code>${esc(m)}</code><button class="btn secondary" data-remove-moderator="${i}">Eliminar</button></div>`).join(''):'<p class="muted">Todavía no has añadido moderadores.</p>'}</div>
      </article>`;
    $('saveAppearance').onclick=()=>persistSettingsPatch({appearance:{theme:$('sTheme').value,accent:$('sAccent').value}});
    $('logout2').onclick=logout;
    $('addTiktokModerator').onclick=async()=>{
      const value=normalizeUsername($('tiktokModeratorInput').value);
      if(!value){toast('Moderador','Escribe un uniqueId de TikTok.','err');return;}
      const next=[...new Set([...mods,value])];
      await persistSettingsPatch({tiktokModerators:next});
      toast('Moderador añadido',`${value} ahora recibirá 🛡️ y será reconocido por el bot de voz.`);
    };
    document.querySelectorAll('[data-remove-moderator]').forEach(btn=>btn.onclick=async()=>{
      const index=Number(btn.dataset.removeModerator); const next=mods.filter((_,i)=>i!==index);
      await persistSettingsPatch({tiktokModerators:next});
    });
  }
