(() => {
  ...
  const pageMeta = {
    ...
    overlays:['EN ESCENA','Overlays'], roulette:['DINÁMICA','Ruleta'], voices:['VOZ','Voces'], points:['ECONOMÍA','Sistema de puntos'], widgets:['WIDGETS','Widgets'], settings:['PREFERENCIAS','Ajustes']
  };
  ...
  let pointsUserRefreshTimer = 0;

  function clearPointsUserRefresh(){
    if(pointsUserRefreshTimer){ clearInterval(pointsUserRefreshTimer); pointsUserRefreshTimer=0; }
  }

  async function renderPoints(){
    clearPointsUserRefresh();
    try {
      ...
      $('view').innerHTML=`
        ...
        <div class="notice">🔥 <strong>Poder de voz</strong> se configura dentro de <strong>Bot de voz → Poder de voz</strong>. Este panel se concentra exclusivamente en la economía y administración de puntos.</div>
        <div class="settings-grid two points-grid">
          <section class="card points-config-card"><div class="section-head"><div><p class="eyebrow">PUNTOS</p><h3>Configuración por plataforma</h3></div><span class="badge-pill">✦</span></div>
          ...
          <section class="card points-manage-card"><div class="section-head"><div><p class="eyebrow">GESTIÓN DE USUARIOS</p><h3>Dar puntos</h3></div><span class="badge-pill">＋</span></div>
            <p class="muted points-manage-help">Busca un usuario que ya haya aparecido en tus actividades. Se consultará su saldo actual y podrás añadir puntos directamente desde aquí.</p>
            <div class="points-search-grid">
              <label>Plataforma<select id="inlineGivePointsPlatform"><option value="tiktok">TikTok</option><option value="twitch">Twitch</option></select></label>
              <label id="inlineGiveIdentityLabel">Unique ID de TikTok<input id="inlineGivePointsUsername" placeholder="@unique_id" autocomplete="off"></label>
              <button class="btn primary points-search-btn" id="inlineFindPointsUser">Buscar usuario</button>
            </div>
            <div id="inlinePointsUserCard" class="points-user-card empty"><div class="points-user-empty-icon">✦</div><div><strong>Sin usuario seleccionado</strong><span>Busca un espectador para ver su información y saldo.</span></div></div>
            <div class="points-award-box">
              <label>Puntos a otorgar<input id="inlineGivePointsAmount" type="number" min="1" step="1" value="100" inputmode="numeric"></label>
              <button class="btn primary" id="inlineGivePoints" disabled>Otorgar puntos</button>
            </div>
            <div id="inlineGivePointsStatus" class="status" aria-live="polite"></div>
          </section>
        </div>`;
      bindPointsPage();
  ...
  function renderPointsPlatformForm(platform){
    ...
    <p class="muted">Los eventos que la plataforma no entrega simplemente no suman puntos. Seguir = 100 puntos y comentario = 2 puntos por defecto en ambas plataformas.${twitch?' Twitch no usa likes ni compartidos.':''}</p>`;
  ...
  function bindPointsPage(){
    ...
    const updateIdentityLabel=()=>{
      const label=$('inlineGiveIdentityLabel'); const input=$('inlineGivePointsUsername');
      if(!label||!input)return;
      if(platform==='twitch'){ label.firstChild.textContent='Nombre de canal'; input.placeholder='canal_twitch'; }
      else { label.firstChild.textContent='Unique ID de TikTok'; input.placeholder='@unique_id'; }
    };

    const renderFoundUser=(u, mode='found')=>{
      const card=$('inlinePointsUserCard'); const grant=$('inlineGivePoints'); if(!card)return;
      if(!u){
        foundUser=null;
        card.className='points-user-card empty';
        card.innerHTML=`<div class="points-user-empty-icon">✦</div><div><strong>${mode==='notfound'?'Usuario no encontrado':'Sin usuario seleccionado'}</strong><span>${mode==='notfound'?'Ese usuario todavía no aparece en las actividades registradas de esta cuenta.':'Busca un espectador para ver su información y saldo.'}</span></div>`;
        if(grant)grant.disabled=true;
        return;
      }
      foundUser={...u};
      const initial=(String(u.displayName||u.username||'?').trim().charAt(0)||'?').toUpperCase();
      const avatarLabel=u.platform==='twitch'?'TW':'TT';
      card.className='points-user-card';
      card.innerHTML=`<div class="points-user-avatar"><span>${esc(avatarLabel)}</span></div><div class="points-user-meta"><strong>${esc(u.displayName||u.username)}</strong><span>${u.platform==='twitch'?'Twitch':'TikTok'} · ${u.platform==='twitch'?esc(u.username):'@'+esc(u.username)}</span><small>Usuario identificado en las actividades de tu cuenta</small></div><div class="points-current"><span>SALDO ACTUAL</span><strong>${Number(u.points||0).toLocaleString('es-PE')} pts</strong></div>`;
      if(grant)grant.disabled=false;
    };

    const refreshFoundUser=async()=>{
      if(!foundUser)return;
      try{
        const q=encodeURIComponent(String(foundUser.username||''));
        const d=await api('/api/points/leaderboard?limit=20&q='+q);
        const fresh=(d.users||[]).find(u=>String(u.platform).toLowerCase()===String(foundUser.platform).toLowerCase() && String(u.username||'').toLowerCase()===String(foundUser.username||'').toLowerCase());
        if(fresh){ const before=Number(foundUser.points||0); foundUser={...foundUser,...fresh}; renderFoundUser(foundUser); if(Number(fresh.points||0)!==before){ $('inlinePointsUserCard')?.classList.add('points-balance-pulse'); setTimeout(()=>$('inlinePointsUserCard')?.classList.remove('points-balance-pulse'),450); } }
      }catch{}
    };
    ...
    $('inlineGivePointsPlatform')?.addEventListener('change',()=>{platform=$('inlineGivePointsPlatform').value;updateIdentityLabel();renderFoundUser(null);});
    ...
    $('inlineFindPointsUser')?.addEventListener('click',async()=>{
      const username=String($('inlineGivePointsUsername')?.value||'').trim().replace(/^@+/,'');
      const card=$('inlinePointsUserCard');
      if(!username){ renderFoundUser(null,'notfound'); card?.classList.add('error'); return; }
      const btn=$('inlineFindPointsUser'); btn.disabled=true; btn.textContent='Buscando…';
      try{
        const d=await api('/api/points/leaderboard?limit=20&q='+encodeURIComponent(username));
        const users=(d.users||[]).filter(u=>String(u.platform).toLowerCase()===platform.toLowerCase());
        const exact=users.find(u=>String(u.username||'').toLowerCase()===username.toLowerCase()) || users[0] || null;
        renderFoundUser(exact, exact?'found':'notfound');
      }catch(e){ renderFoundUser(null,'notfound'); const c=$('inlinePointsUserCard'); if(c){c.className='points-user-card error';c.innerHTML=`<div><strong>No se pudo consultar</strong><span>${esc(e.message||'Intenta de nuevo.')}</span></div>`;} }
      finally{ btn.disabled=false; btn.textContent='Buscar usuario'; }
    });

    $('inlineGivePointsUsername')?.addEventListener('keydown',e=>{if(e.key==='Enter')$('inlineFindPointsUser')?.click();});
    $('inlineGivePointsAmount')?.addEventListener('input',e=>{e.target.value=e.target.value.replace(/[^0-9]/g,'');});

    $('inlineGivePoints')?.addEventListener('click',async()=>{
      if(!foundUser)return;
      const amount=Math.max(1,Math.floor(Number($('inlineGivePointsAmount')?.value)||0));
      const status=$('inlineGivePointsStatus'); const btn=$('inlineGivePoints');
      if(!amount){if(status){status.className='status err';status.textContent='La cantidad debe ser mayor que 0.';}return;}
      btn.disabled=true;
      try{
        const result=await api('/api/points/user',{method:'POST',body:JSON.stringify({platform:foundUser.platform,username:foundUser.username,displayName:foundUser.displayName||foundUser.username,amount})});
        const previous=Number(foundUser.points||0); const fresh={...foundUser,...(result.account||{}),points:previous+amount}; foundUser=fresh; renderFoundUser(fresh);
        if(status){status.className='status ok points-success';status.innerHTML=`✓ <strong>Se agregaron ${amount.toLocaleString('es-PE')} puntos correctamente.</strong> Nuevo saldo: ${Number(fresh.points).toLocaleString('es-PE')} pts`;}
        $('inlineGivePointsAmount').value='';
        const card=$('inlinePointsUserCard'); card?.classList.add('points-balance-pulse'); setTimeout(()=>card?.classList.remove('points-balance-pulse'),450);
      }catch(e){if(status){status.className='status err';status.textContent=e.message||'No se pudieron añadir los puntos.';}}
      finally{btn.disabled=false;}
    });

    updateIdentityLabel();
    pointsUserRefreshTimer=setInterval(refreshFoundUser,5000);
  }
