...
const AVATAR = (seed) => `https://api.dicebear.com/8.x/initials/svg?seed=${encodeURIComponent(seed || "StreamFusion")}`;
...
function renderTopbar() {
  ...
  els.tiktokState.textContent = tiktok.connected ? `Conectado ${tiktok.username ? `@${tiktok.username}` : ""}` : "Desconectado";
  els.twitchState.textContent = twitch.connected ? `Conectado ${twitch.username || ""}` : "Desconectado";
}

function renderConnectionStrip() {
  const showTikTok = !state.session.tiktok.connected;
  const showTwitch = !state.session.twitch.connected;
  els.connectStrip.classList.toggle("hidden", !(showTikTok || showTwitch));
  els.connectStrip.children[0].style.display = showTikTok ? "block" : "none";
  els.connectStrip.children[1].style.display = showTwitch ? "block" : "none";
}
...
function bootstrap() {
  ...
  socket.on("event", (data) => {
    ...
    const item = {
      platform,
      type: String(data?.type || "system").toLowerCase(),
  ...
  // Keep the top cards visible even if a platform is only partially connected.
