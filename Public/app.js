let socket = null;
let sfAuthToken = localStorage.getItem("sf_auth_token") || "";
...
function openOverlay(view) {
  ...
  const owner = encodeURIComponent(sfCurrentUser?.id || "public");
  const url = `${file}?view=${encodeURIComponent(safeView)}&owner=${owner}`;
...
function pushChat(data) {
  ...
  item.eventId = String(data?.eventId || data?.messageId || data?.msgId || "").trim();
  const fingerprint = item.eventId || `${item.platform}|${normalizeUsername(item.user)}|${String(item.message||"").trim()}|${Math.round(Number(item.timestamp||Date.now())/900)}`;
  item._fingerprint = fingerprint;
  if (state.chat.some((existing) => existing._fingerprint === fingerprint)) return;
  rememberSupporter(item);
  state.chat.push(item);
  if (state.chat.length > 360) state.chat.splice(0, state.chat.length - 360);
  state.chatScroll.follow = true;
  state.chatScroll.unread = false;
  renderChat();
  syncChatNotice();
...
async function bootstrap() {
  ...
  if (!authToken) {
    window.__SF_AUTH_REQUIRED__ = true;
    window.dispatchEvent(new CustomEvent("sf-auth-ready"));
    return;
  }
  try {
    ...
    sfAuthToken = authToken;
    sfCurrentUser = me.user || null;
    window.__SF_CURRENT_USER__ = sfCurrentUser;
    socket = io({ auth: { token: sfAuthToken } });
    window.__SF_SOCKET__ = socket;
    loadSettingsToUI();
    renderAll();
    bindEvents();
    bindChatScroll();
  ...
  socket.on("chat", (data) => {
    ...
    pushChat({
      ...
      eventId: data?.eventId || data?.messageId || data?.msgId || "",
      badges: data?.badges || [],
      emotes: data?.emotes || "",
      color: data?.color || "",
      timestamp: data?.timestamp || Date.now(),
  ...
  socket.on("stats", (data) => {
    ...
    console.error("No se pudo iniciar StreamFusion:", error);
    localStorage.removeItem("sf_auth_token");
    window.__SF_AUTH_REQUIRED__ = true;
    window.dispatchEvent(new CustomEvent("sf-auth-ready"));
  }
}
window.__SF_BOOTSTRAP__ = bootstrap;
bootstrap();
