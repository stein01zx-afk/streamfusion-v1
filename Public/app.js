(() => {
  ...
  function renderOverlays() {
    const unifiedOverlayPath='overlay.html';
    $('view').innerHTML=`<div class="intro"><h2>Overlays</h2><p>Chat, eventos y regalos usan un único enlace por cuenta. Todos leen la misma personalización y cualquier cambio se aplica al mismo overlay.</p></div><div class="overlay-status"><span class="status-pill ${state.connection==='online'?'on':''}"><i></i>${state.connection==='online'?'Conectado al stream':'Sin conexión'}</span>${['tiktok','twitch'].map(p=>`<span class="channel-state ${isConnected(p)?'on':''}">${p==='tiktok'?'TikTok':'Twitch'} · ${isConnected(p)?'ON':'OFF'}</span>`).join('')}</div><div class="overlay-grid">${overlayCard('Overlay unificado','overlay.html','Un solo enlace para Chat, Eventos y Regalos. El contenido se cambia desde el propio overlay y toda la personalización pertenece a esta cuenta.')}${overlayCard('Ruleta','roulette-overlay.html','Ruleta overlay independiente, usando la misma conexión de cuenta.')}</div>`;
