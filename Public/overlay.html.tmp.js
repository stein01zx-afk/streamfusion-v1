  ...
  missa_death_note: { label: "Missa Death Note", id: "c6aad54044814847aa2e9c272a2b4815" },
...
function adjustOverlayZoom(delta){
  ...
  const flags = [
    ...
    voiceBot.profanityFilter ? "antigroserías" : null,
  ].filter(Boolean).join(" · ");
...
function cleanVoiceText(text, { isName = false } = {}){
  ...
  return out;
    ...
    function syncVoiceBotUI(){
      ...
      if (voiceSelect) {
        const currentVoiceKey = voiceBot.voiceKey in voiceCatalog ? voiceBot.voiceKey : "verity";
        if (voiceSelect.dataset.voiceCatalogReady !== "1") {
          voiceSelect.innerHTML = Object.entries(voiceCatalog)
            .map(([key, voice]) => `<option value="${esc(key)}">${esc(voice.label)}</option>`)
            .join("");
          voiceSelect.dataset.voiceCatalogReady = "1";
        }
